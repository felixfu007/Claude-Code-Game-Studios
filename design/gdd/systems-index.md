...
| ADRs Accepted | **4**(2026-09-01 更新 —— ADR-0001 戰棋查詢原子性、ADR-0005 游標裝置權威**同批核准**,依據五個核准條件逐條成立,含**首次執行的 `TD-ADR` 架構總監覆核**與其後的窄範圍複驗;引擎探針 0001 四支全過、0005 六項關五項)。**五份中只剩 ADR-0004 為 `Proposed`,而它依一年計畫第七節明確不在 12 個月範圍內**(「不在最短路徑上,維持 `Proposed` 即可,不要再投入」)。<br>**歷程**:0(2026-08-18 建立五份時)→ 2(2026-08-25,ADR-0002/0003)→ 4(2026-09-01)。<br>⚠️ **核准數不等於可開工數**:#4 戰棋系統雖然 ADR-0001 已核准,仍受其自身 **OQ-2**(我方基準數值表無人擁有)阻擋 |
