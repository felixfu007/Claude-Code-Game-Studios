...
| 6 | 技能卡牌系統(含好感度對話卡牌) | Gameplay | MVP(**範圍收窄 2026-08-25**:MVP 只需好感度對話卡牌這一種子類型;「完整卡牌技能池」是 `game-concept.md` 的 MVP 明確不做項) | **Revised(2026-09-08 第一輪 `/design-review` full 模式,六專家 + `creative-director`,判 MAJOR REVISION NEEDED,17 叢集同輪修訂完畢)**<br>🔴 **17 叢集全部落在 native design(review-added 0),accretion signal 未觸發** —— 依 Phase 0b 這是好消息:問題在設計內容本身,不是治理層打轉。<br>🔴 **開工前三把鎖,其中兩把是本輪才發現的**:①我方基準數值表(#4 的 OQ-2,已指派未執行);②**~~ADR-0001 修訂~~ ✅ **2026-09-09 完成**(第一次修訂:計數器改名 `combat_state_version` 並移交 `BattleState`、寫入路徑改以 mutator 列舉補齊至六條、新增寫入守衛)。⚠️ **原本「本文寬 1 處、窄 4 處」的描述是錯的框架** —— 兩種讀法都沒涵蓋打牌,實際是機制管轄範圍不夠大。**它另生出一件:把 `TurnOrder` 收進 `BattleState` 的重構**;③戰棋系統須新增「基準值 vs 有效值」存取層(管理者已許可)。<br>🔴 **管理者五項裁決同日落地**:丙類留在 MVP、允許改動戰棋系統、打牌算改變盤面、維持出牌數不限、**丙類不得翻轉 canon 極性** —— 最後一項導出未預期結果:**丙類對傷害的影響變成零,而那是設計**,分工收斂為「甲類負責戰鬥、丙類純粹負責劇情」。<br>🔴 **行數超出 400 上限,已登記為第三份明文例外**(計畫檔第六節②;該處同時記錄「三份例外之後實務判準已是個案裁決」這項知情代價)。<br>⚠️ **依劑量規則只剩一輪審查額度。** 審查全紀錄見 `design/gdd/reviews/skill-card-system-review-log.md`<br>✅ **2026-09-09:三把鎖全部關閉,`/ux-design` 亦執行完畢 → `design/ux/skill-card-play.md`(本專案第一份逐畫面 UX 規格)。實作故事引用該檔,不得引用 GDD。**<br>🔴 **但同日長出一把只擋介面的新鎖(GDD OQ-13)**:管理者裁決把「結束回合」移出 `Esc`/手把 `B` 讓給取消,而**結束回合的新家(一個選單)不存在也無人擁有** —— 那兩個綁定目前是玩家唯一能把回合交給敵方的手段,只做前半會讓戰鬥永遠停在自己的回合。**邏輯與測試可先做,打牌介面的取消鍵不行。**<br>🔴 **OQ-1 的關法值得記住**:那張基準數值表**九天前就有擁有者**,上游 `#4` 的 OQ-2 沒跟上 2026-08-31 的指派,本文件照抄,2026-09-02 還據它重複指派一次。**「已指派、等交件」把責任指向外部,而外部不會來反駁** —— 它不會編譯失敗、不會測試變紅、不會跟任何數字矛盾。**看到這種措辭先 grep 產出物是否已存在。** | design/gdd/skill-card-system.md | 好感度數值池、戰棋移動與交戰系統。**⚠️ 本輪新增反向依賴:戰棋系統(#4)須為本系統新增數值存取層,並在其設計文件補一節** |
...
| ❌ **作廢,非解決(🔴 2026-09-03,管理者裁決 TR-cursor-016)** 原義務:全域裝置狀態指示元件(待機指示)由哪個下游系統擁有/渲染尚未指派。**本義務已隨其標的物(全域裝置狀態指示元件本身)一併取消而消失,不是被履行**——管理者裁決取消「全域每裝置待機指示」需求,見 `cursor-highlight-state.md` Status 區塊 2026-09-03 條目與 `production/session-state/active.md` 2026-09-03 條目(第三十二批)。戰鬥 HUD、支援對話 UI、好感度視覺呈現 UI 三個原候選承接系統**不再需要**指派任何擁有者。 | cursor-highlight-state.md Visual/Audio Requirements「唯一例外」退役註記(原登記來源:2026-08-05 第九輪新增,第十輪修正措辭) | 無(原候選:戰鬥 HUD、支援對話 UI、好感度視覺呈現 UI——已因義務取消而不再適用) | 不再需要任何下游系統指派擁有者——沒有元件需要被擁有 |
...
| 🔴 **「結束回合」需要一個新家(2026-09-09 `/ux-design skill-card-play` 發現;✅ 同日規格已產出)。** 管理者裁決:`Esc` / 手把 `B` 讓給打牌流程的取消鍵,結束回合改由選單內的選項發起(理由:太容易誤觸、白白浪費一回合)。**而 `battle_end_phase` 那兩個綁定目前是玩家唯一能把回合交給敵方的手段** —— 只做前半,戰鬥會永遠停在玩家自己的回合。⚠️ **本專案已在同樣這兩個動作上踩過一次同形狀的問題**(`battle_screen.gd` 逐字記載:原綁的 `ui_cancel` 沒有手把綁定,「would otherwise leave a gamepad-only player unable to ever hand the turn to the enemy」) | `design/ux/skill-card-play.md` Interaction Map + `design/ux/battle-menu.md` + `design/gdd/skill-card-system.md` OQ-13 | ✅ **2026-09-09 同日指派並產出規格:`design/ux/battle-menu.md`**(管理者裁決「選單要現在做」)。🔴 **它同時關閉一個更舊的懸空點** —— 游標系統的 `suspend_arbitration()` 自 2026-08-11 寫下起就假設有一個呼叫方,而那個呼叫方從未被指派;**該畫面就是它**。⚠️ **該畫面另查出一件會被踩的事**:暫停裁定**不得用 `SceneTree.paused` 或 `process_mode`**(`docs/registry/architecture.yaml` 的 `cursor_arbitration_suspension_gate` 明文排除),**而那是做暫停選單的第一直覺**。**實作待排;動作歸屬見該檔 BM-1** | 存在一條鍵盤與一條手把皆可完成「結束回合」的路徑,且取消鍵已重綁 —— 驗收條件為 `skill-card-play.md` 的 **AC-U12** 與 `battle-menu.md` 的 **AC-M1**,**同一條要求的兩側,兩份都要過** |
...
| ADRs Accepted | **4**(2026-09-01 更新 —— ADR-0001 戰棋查詢原子性、ADR-0005 游標裝置權威**同批核准**,依據五個核准條件逐條成立,含**首次執行的 `TD-ADR` 架構總監覆核**與其後的窄範圍複驗;引擎探針 0001 四支全過、0005 六項關五項)。**五份中只剩 ADR-0004 為 `Proposed`,而它依一年計畫第七節明確不在 12 個月範圍內**(「不在最短路徑上,維持 `Proposed` 即可,不要再投入」)。<br>**歷程**:0(2026-08-18 建立五份時)→ 2(2026-08-25,ADR-0002/0003)→ 4(2026-09-01)。<br>⚠️ **核准數不等於可開工數**:#4 戰棋系統雖然 ADR-0001 已核准,仍受其自身 **OQ-2**(我方基準數值表;✅ 2026-09-02 已指派 `systems-designer`,表尚未產出)阻擋 |
