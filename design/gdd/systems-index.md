...
| 🔴 **「結束回合」需要一個新家,而那個家不存在也無人擁有(2026-09-09 新增,`/ux-design skill-card-play` 發現)。** 管理者裁決:`Esc` / 手把 `B` 讓給打牌流程的取消鍵,結束回合改由選單內的選項發起(理由:太容易誤觸、白白浪費一回合)。**而 `battle_end_phase` 那兩個綁定目前是玩家唯一能把回合交給敵方的手段** —— 只做前半,戰鬥會永遠停在玩家自己的回合。⚠️ **本專案已在同樣這兩個動作上踩過一次同形狀的問題**(`battle_screen.gd` 逐字記載:原綁的 `ui_cancel` 沒有手把綁定,「would otherwise leave a gamepad-only player unable to ever hand the turn to the enemy」) | `design/ux/skill-card-play.md` Interaction Map + `design/gdd/skill-card-system.md` OQ-13 | 🔴 **待指派** —— 屬回合結構/戰棋系統(#4),不屬技能卡牌系統(#6)。⚠️ 互動模式庫 Gaps 表對「暫停/模態選單」明文登記「無明確擁有者/不存在」,且游標系統的 `suspend_arbitration()` 假設有一個從未被指派的呼叫方 | 存在一條鍵盤與一條手把皆可完成「結束回合」的路徑,且取消鍵已重綁 —— 驗收條件為 `design/ux/skill-card-play.md` 的 **AC-U12**(該條唯一的用途就是防止這件事交付到一半) |
