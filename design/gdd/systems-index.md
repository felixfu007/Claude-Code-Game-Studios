...
| 6 | 技能卡牌系統(含好感度對話卡牌) | Gameplay | MVP(**範圍收窄 2026-08-25**:MVP 只需好感度對話卡牌這一種子類型;「完整卡牌技能池」是 `game-concept.md` 的 MVP 明確不做項) | **In Design(2026-09-08:13 節初稿全部完成,尚未經任何 `/design-review`)**<br>🔴 **同日二次裁決擴大 MVP 範圍:「暫時性戰鬥數值」(甲類,收窄為 `ATK`/`DEF`,不含移動力)納入實作**,原本的「只做永久好感度寫入」被推翻。**撤銷原因是查證發現的事實,不是偏好改變**:加成公式只讀關係的正負與距離、**不讀好感度數值大小**,故純好感度寫入的卡牌在戰鬥中實質無效 —— 支柱那句「差距要靠好感度佈局的加成**與卡牌效果**補足」的後半會不兌現。詳見該 GDD Core Rules 一之二。<br>🔴 **擋著開工的兩件事**:①**我方基準數值表尚未產出**(#4 的 OQ-2,2026-09-02 已指派 `systems-designer`,工作未執行);②**戰鬥中能不能手動存檔沒人問過**(本次新發現,只需一次裁決)。<br>⚠️ **本文件單方面宣告關閉了三個上游 OQ(#4 的 OQ-4 一半與 OQ-19、#5 的 OQ-3),而上游三份尚未更新** —— 兩側同步是待辦第一項,見該 GDD Dependencies。<br>⚠️ **本系統的繼承義務為 6 項而非既有文件所載的 5 項**(第 6 項為 #4 的 OQ-19 反擊機制,撰寫期查證才發現) | design/gdd/skill-card-system.md | 好感度數值池、戰棋移動與交戰系統 |
