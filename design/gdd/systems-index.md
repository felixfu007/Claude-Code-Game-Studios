...
| **`source_absence`(2026-08-11 第七輪新增,D-1 修法,回應 `/review-all-gdds` 發現的可逆性矛盾)**:拿 `source_absence` 判定互斥獨佔內容,僅得於戰役終局判定當下呼叫一次,不得在戰役中途做「永久關閉」式的即時宣告;戰役進行中若需要呈現現況,僅能以**可逆的事實**呈現(例如「至今未曾透過此來源互動」),不得宣稱任何內容因此永久關閉——不適用上一列的「單調不可逆」豁免與其揭露義務 | `game-concept.md`「範圍排除與揭露義務」D-1 修法(2026-08-11)+ `affinity-data-pool.md` 3g 段落「可逆性明文承認」+ AC-80 | 敘事解鎖與結局分支系統、好感度視覺呈現 UI | 該系統 GDD 的獨佔內容判定邏輯明確僅於終局呼叫 `source_absence`,且中途呈現(若有)採用非永久性措辭,通過 `game-concept.md` 常設風險段落既有的沉默可辨識性重測協定不受本列影響(兩者為獨立驗證維度) |
