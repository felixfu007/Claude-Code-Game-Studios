...
| S13(新) | 完備性執行紀錄表仍以 **Core Rule** 為檢查單位,未升級為逐義務檢查——第五輪原訂的升級觸發條件(「下一次實質修訂 Core Rules 內容時一併評估是否值得投入」)於第六輪已成立卻從未執行,第七輪七項阻擋中有五項落在該段自己描述的盲區內。第七輪依使用者裁決 U-3 乙案,將該模糊條款改寫為可稽核的機械觸發規則,但**全表逐義務升級本身尚未進行** | R7 | 已將觸發條件改寫為硬性可稽核條款(「任何一次修訂 Core Rules 內部義務句的輪次,須於同輪對受影響規則執行逐義務三件套核對,並將核對結果寫入該輪 review-log 條目;未執行核對的輪次不得宣告該規則三件套齊全」)+ 本 Ledger 列 | 是——`save-system.md` 完備性執行紀錄表「升級觸發條件改寫為可稽核版本」段落 | **畢業條件與一般 Ledger 列不同**:本列不以「連續 3 輪未處置即升級」計數,改以**實證訊號**畢業——若後續任一輪出現阻擋落在**未被該輪修訂過**的規則上(即機械觸發條款無法涵蓋的情境),即為全表逐義務升級的實證觸發訊號,屆時本列升級為阻擋候選;若連續 3 輪的阻擋皆落在被修訂過的規則上(證明機械觸發條款的覆蓋範圍與真實失敗分布匹配),本列可畢業 |
...
**收斂性判讀:本輪是收斂的,且是六輪以來訊號最清楚的一次**。依據三點:(1)阻擋數與性質同時下降——7 項中 5 項為純傳播失敗(改文字即可、零設計判斷),第六輪十項修訂有八項經七位專家對抗性覆核後零發現,是本文件史上單輪修訂正確率最高的一次;(2)**本輪首次出現「單一結構性改動可防止多數阻擋再發」的形態**——B1/B3/B4/B5 共享同一個成因(規則級檢查粒度放行了義務級缺口),U-3 乙案一次關閉整類,前六輪從未出現這種成因集中度;(3)升級決策被證明正確(見上方背景)。阻擋數:24→9→11→8→11→10→**7**。

**反向訊號(必須點名,已寫入第八輪退場條件)**:第六輪標頭有**兩處**「已完成」的自我宣稱經查證不實——(a)「Dependencies 章節三處落差修正(生命週期通知路徑數同步…)」實際只修了 Interactions 的對應條目,真正的 Dependencies 表格列至今停留在最陳舊的「**兩條路徑**」版本,連 2026-08-07 擴充的三條都沒跟上;(b)AC-76 的呈現規範宣稱「已同步登記至 `systems-index.md` Cross-System Obligations Registry」,經逐列查證**該列從未存在**。兩者與第三輪 R3-B5 的教訓(「R2 ledger『已解決』判定只驗證了新交付物存在,未驗證舊矛盾敘述已移除」)**完全同型,發生在四輪之後**。結論:**本文件家族的「修訂完成」自我宣稱不可信,每輪的修訂清單本身都應被下一輪當成待查證的宣稱,而非已知事實。**

**本輪使用者裁決(三項,全數採納 creative-director 推薦選項)**:
1. **U-1(路徑〔四〕的唯讀存取管道)→ 甲案:延伸唯讀存取介面可用條件**。由「拒絕讀取狀態」擴大為「拒絕讀取狀態,**或**待遷移狀態且已滿足重複回寫失敗條件且當下無進行中的分步狀態機」。理由:消除保護等級倒掛、沿用既有介面不新增機制、保留介面分類表「無重入疑慮」的原始論證。否決乙案(新增獨立槽狀態——新狀態就是新接縫,是本文件六輪的教訓)與丙案(接受現況加範圍聲明——等於明文承認倒掛,而該倒掛違反本文件自己的嚴重度排序邏輯)。
2. **U-2(陣亡配對 `source_absence_cc` 的揭露處置)→ 丙案:陳述事實但不使用關閉語言**。以角色語言陳述為真的事實(例如「你和她之間,再也不會有並肩作戰時的那種話了」),不談路線、不談關閉、不談機制。理由:滿足事實層透明,但不在死亡時刻插入一次可得性的損失結算;此語域與本專案 2026-08-10 第八輪對陣亡議題的既有裁決同源,是延用而非新發明。否決甲案(比照 `total_churn` 的無條件關閉揭露——把哀悼變成清算)與乙案(沉默——系統一路知情未告,是 D-1 當初要修掉的失格模式的鏡像)。**重要澄清:此裁決不推翻第六輪使用者的 A+ 裁決**,「終局判定 + 過程中誠實告知」方向維持,修正的是其前提句的全稱性與適用粒度。
3. **U-3(完備性執行紀錄表是否升級為逐義務檢查)→ 乙案:本輪不升級,但把觸發條件改寫為可稽核版本**。理由:本輪五項阻擋全部落在**被修訂過的**那四條規則上,沒有一項落在未被觸碰的規則上——乙案的覆蓋範圍恰好對準真實的失敗分布,甲案(全表升級)多付的成本買的是尚未發生的風險。全表升級不取消,改由 Ledger S13 累積實證後重新評估。

**Revision list(blocker → fix applied,同一 session 內完成)**:

| # | 阻擋項摘要 | 修訂內容 |
|---|---|---|
| R7-B1 | **路徑(四)未傳播至六處位置**(systems-designer 1+2、game-designer 2 各自發現其中一部分,creative-director 查證新增第 5、6 處)——第六輪新增路徑(四)時只改了 Core Rules #5 的路徑清單本身 | 六處全數同步:(1)Core Rules #5 導言句「三條」→「四條」;(2)權杖式語意段「以下三條路徑」→「四條」;(3)Core Rules #16 矩陣生命週期通知列「三分支」→「四分支」並列出各路徑的保證強度;(4)**States and Transitions「待遷移」列離開條件**由「成功或失敗」二分法改寫為三結局模型(成功且回寫完成→有效 / Core Rules #6 拒絕→拒絕讀取 / 路徑〔四〕→**不離開本狀態**),並明文指出原二分法會讓照表實作者在「鏈執行成功」時讓槽離開一個沒有目的地的狀態,是構造上的死路;(5)**Dependencies 表好感度數值池列**由「**兩條路徑**」改寫為四條路徑完整描述(全文件最陳舊的一處,七位專家皆未發現);(6)**`systems-index.md` Registry 補上 AC-76 的登記列**(第六輪宣稱已登記但該列從未存在)。(5)(6)兩處已寫入第八輪退場條件第 2 項 |
| R7-B2 | **第六輪 cursor 端 F2-1/F2-2 修法缺乏可歸因的驗證**(qa-lead 5+6 合併):F2-2 甲/乙分支零專屬 AC;AC-60 與 AC-59 共用暫停選單情境,導致 AC-60 通過可被「暫停凍結」完整解釋,**登記制判準本身實質從未被驗證**(一個只實作暫停讓路、完全沒實作登記制的系統會讓兩條 AC 都通過) | `cursor-highlight-state.md` 新增 L 節三條 AC:AC-61(甲分支:舊表面存在時讀檔,交接呼叫須在卸載前發生)、AC-62(乙分支:複用 Core Rules #6 初始狀態流程,且與「新開局進入戰鬥」的狀態轉換序列完全相同——驗證「走完全相同程式碼路徑」的設計宣稱而非僅文件敘述)、AC-63(丙分支,見 R7-R4);AC-60 測試情境由「暫停選單」改為「不觸發 `SceneTree.paused` 的未登記表面」,並明文記載兩條 AC 情境必須互斥的理由 |
| R7-B3 | **AC-68/AC-73 引用了不存在的診斷能力**(qa-lead 1):兩條 AC 皆宣稱可透過「Core Rules #15 甲類診斷介面」觀測 begin/end 呼叫時序,但甲類三項介面(遷移呼叫計數器、遷移呼叫順序記錄、主執行緒斷言)**皆不監測生命週期通知呼叫**——完備性表對這兩條 AC 的「今日可執行」判定建立在一個不存在的能力上 | Core Rules #15 甲類新增**第四項:生命週期通知呼叫記錄**(記錄每次 begin/end 的呼叫類別、權杖識別、槽識別、相對於遷移鏈與兩階段回寫的時序位置,並須能判定兩次呼叫之間是否跨越事件迴圈邊界——後者是 AC-73 對應斷言的唯一觀測手段);「遷移呼叫順序記錄」補上範圍限定(不涵蓋對外部系統的介面呼叫);矩陣 #15 甲類列補上對 #5 四條終止路徑的依賴;新增 **AC-78** 驗證該介面本身;介面分類表「診斷介面(甲類三項)」→「四項」 |
| R7-B4 | **AC-10a 的「今日可執行」理由與其驗證機制矛盾**(godot-specialist 1):完備性表 K 列以單一句「全數以測試替身模擬 I/O 層級失敗回傳值即可構造」涵蓋六條 AC,該句對 AC-10a 為假(它驗證 OS 層級 rename 原子性與行程終止,須實際終止 headless 行程)。**而 AC-10 拆分為 AC-10a/AC-10b 的全部目的正是分開這兩種依賴不同保證機制的驗證,本表在同一輪用一句方法論宣稱把它們重新合併回去**,等於在紀錄表層面撤銷了該輪的修法意圖 | 表二 K 列拆分為兩列(AC-10a 獨立成列並註明其方法論差異與拆分意圖被撤銷的事實;其餘五條維持測試替身方法論並註明 AC-73 另需甲類第四項介面);新增表二「**『今日可執行』的判定粒度限制**」段落:同一章節內若存在方法論不同的 AC 須拆列分述,不得以單一句子概括——理由與表一的規則級假綠燈同型 |
| R7-B5 | **Core Rules #14「確保」定義結構性排除步驟四且未聲明理由**(qa-lead 2):規則寫「步驟 2-4a」、AC-75 寫「步驟 1-4a」,兩者一致地把步驟四(`.bak`→`.prev.bak` 的 rename)排除在檢查義務外。步驟四正是第四輪認定「若結構性無法完成,該槽後續所有寫入會被步驟零永久卡住」的那一步——一次未被偵測的步驟四失敗會讓殘留 `.bak` 被步驟零反覆補完反覆失敗,且沒有任何機制會發現 | Core Rules #14 步驟 1「確保」範圍改為明文列舉「步驟 1、2、3、4a、4」(含 4a 的刪除呼叫),新增「範圍涵蓋步驟四,不得排除」段落說明排除的具體危險;AC-75 同步擴大並依 AC-38 既有範式**逐步驟列舉**中止後的檔案狀態(五個步驟各自的 `.dat`/`.bak`/`.prev.bak`/`.pre_migration.bak` 精確斷言),取代原「依步驟而定」的概括措辭(一併解決 R7-R3) |
| R7-B6 | **路徑(四)的槽狀態與唯讀存取介面可用條件不相交**(game-designer 1,**本輪兩項真設計缺陷之一**):路徑(四)槽維持「待遷移」,而唯讀存取介面定義上僅於「拒絕讀取」可用(Core Rules #13、#15 分類表、AC-73 三處獨立文字構成閉環)→ 持續回寫失敗的玩家**沒有任何唯讀檢視/匯出管道**。creative-director 重新框定其嚴重度來源:真正的問題是**保護等級倒掛**——本系統對一個「資料完好、只是寫不進去」的槽提供的保護,低於對一個「資料確實已損毀」的槽(後者有唯讀檢視、匯出、分級告知、復原前景語句),直接違反 Player Fantasy 自訂的嚴重度排序邏輯;且在權限錯誤/唯讀媒體/主機沙盒提交失敗情境下玩家無法自行解除 | **使用者裁決 U-1 甲案**:Core Rules #5 路徑(四)新增「重複回寫失敗的判定門檻與唯讀存取管道」段落,於**規則層**定義「連續兩次」門檻(含計數歸零條件、刻意不持久化的理由）；Core Rules #13 唯讀存取可用條件擴大為「拒絕讀取狀態,或待遷移狀態且已滿足重複回寫失敗條件且當下無進行中的分步狀態機」(兩個附加條件缺一不可,後者保留分類表「無重入疑慮」的原始論證);Core Rules #15 介面分類表同步;States 表待遷移列同步;Interactions 新增「須同時提供唯讀存取入口」條款含**與其他三類區分的復原前景語句**(此為本文件唯一一處玩家動作確實有效的情境);新增 **AC-77**(三組 GIVEN:連續兩次後開放、僅一次時不開放、狀態機執行中維持 AC-59 既有行為);`systems-index.md` 補登記列 |
| R7-B7 | **`source_absence_cc` 在陣亡配對上實為單調不可逆,D-1 修法的全稱前提有具體反例**(narrative-director 1,併入其發現 4 的一般化,**本輪最嚴重發現**):`game-concept.md` D-1 修法的前提句「它不是單調不可逆特徵」是無條件全稱宣稱,但依 `affinity-data-pool.md` Core Rules #2/AC-63,陣亡配對的 `cc` 來源寫入被永久拒絕,而該分量翻轉的唯一途徑正是該來源發生新記錄 → 對陣亡配對構造上不可翻轉。後果:D-1 第(三)款「不得宣稱任何內容因此永久關閉」會**強制系統說出一句實質為假的話**,是本文件自陳最毒失敗類別的鏡像版本。creative-director 另查證發現 **AC-80 對此反例結構性空轉**(其 GIVEN「之後該來源實際發生一筆新記錄」在該組合下不可構造,永遠通過、永遠偵測不到) | **使用者裁決 U-2 丙案**,三份文件同步修訂,**且依 narrative-director 發現 4 以通則而非枚舉陣亡個案書寫**(理由:資料層可逆 ≠ 玩法結構可逆,陣亡只是這個預設失效的第一個實例,枚舉會讓下一個實例重現同一個洞):`game-concept.md` D-1 修法新增「三項修正」——修正一(可逆性須**逐(配對, 來源)判定**,任何使來源 X 對配對 p 永久不可寫入的玩法結構事件使該分量轉為單調不可逆並適用豁免資格;承接系統須逐一列出本作所有此類事件)、修正二(不可逆子情境的揭露採**角色語言陳述事實、不使用關閉語言**,含硬性禁用語彙清單)、修正三(中途揭露義務的強制性明確化:可逆子情境**許可非強制**、不可逆子情境**強制**——解決 R7-R8「得以」vs「此告知義務」的 modality 矛盾);`affinity-data-pool.md` 3g 新增「可逆性的正確範圍:資料層可逆 ≠ 玩法結構可逆」段落(含本系統自身規則造成封鎖的明文列舉義務)+ AC-80 補覆蓋範圍限制 + 新增 **AC-81**(反面測試:陣亡配對 × `cc` 恆為 `absent_confirmed`,且驗證封鎖僅及於 `cc`、不擴及 `sc`/`se`);`systems-index.md` 第 178 列改寫為甲/乙兩類分述 |

**Recommended revisions applied(14 項,全數同批完成)**:
- **R1【三合一】**Core Rules #8 新增「操作原子性:須為同一次序列化操作產出的同一份緩衝區」段落——合併 godot-specialist(表示法決定論)+ security-engineer(操作原子性)+ security-engineer(威脅模型未回頭交叉引用 Core Rules #4 前提)三項發現,一句修法收攏;明文說明兩個失敗方向(重複序列化→安全失敗但製造無法重現的偽損毀;雜湊作用於物件圖→不安全失敗)與其對 Core Rules #4 單執行緒模型的前提依賴
- R2(systems-designer 3)Core Rules #5 路徑(四)新增「與路徑(三)的邊界——失敗訊號的形式決定路徑歸屬」:I/O 失敗若以未分類例外形式拋出,落入路徑(三)、**不享有**路徑(四)的 end 呼叫保證
- R3(systems-designer 4)AC-75 逐步驟列舉(併入 R7-B5)
- R4(systems-designer 5)`cursor-highlight-state.md` Core Rules #7 新增**(丙)讀檔流程被取消、返回原表面**分支——`save-system.md` 路徑(三)自己點名的情境,第六輪只涵蓋甲/乙兩條前進路徑;新增 AC-63;`systems-index.md` 第 166 列觸發情境再擴充
- R5(game-designer 4)乙分支新增「已知體感落差:高亮位置與滑鼠實際螢幕位置不同步」明文登記,**維持建議級不修改行為**,依 2026-08-11 第十二輪使用者對滑鼠奪權子機制的既有裁決(回合制戰棋、投入產出比遞減),並指出 Core Rules #3 既有奪權機制已提供自然解法
- R6(game-designer 3)`game-concept.md` D-1 修法新增「相關但獨立的建議級落差」段落:即使真正可逆的子情境,玩法結構仍可能提早耗盡互動機會窗口,形成「規則層可逆、體感上不可逆」
- R7(narrative-director 2)同上段落補入 `source_absence_se` 的能動性措辭排除(承接 3g 既有排除條款,第(三)款範例措辭僅對 `cc`/`sc` 適用)
- R8(narrative-director 3)modality 矛盾(併入 R7-B7 修正三)
- R9(performance-analyst 1)Core Rules #8 新增「可選讀回比對的耗時歸屬」+ AC-36 新增第四段子成本分桶——避免可選手段的 I/O 成本汙染 `save_write_max_duration_ms` 校準基礎、進而讓寬限上限規則依錯誤來源觸發 Core Rules #4 重新評估
- R10(performance-analyst 2)Tuning Knobs 新增「路徑(四)持續失敗下的重複重跑成本」:有界(門檻 × 單次遷移鏈耗時)但未量化,明文承認「乘以重試次數」維度存在
- R11(security-engineer 1)Core Rules #14 耐久性範圍聲明新增「第三類終止情境:OS 當機/強制重啟但電源未斷」——正確二分應為「資料是否已離開 OS 快取抵達儲存裝置」,而非「進程死了還是電源斷了」;不改變保證範圍,只更正其描述方式
- R12(qa-lead 3)「連續兩次」門檻由 AC 層提升至規則層定義(併入 R7-B6)
- R13(qa-lead 4)AC-74 新增「與 AC-75 的關係澄清,避免測試建構衝突」:本 AC 的測試替身須明確標註為「防線繞過模式」,兩條 AC 分別驗證縱深防禦的兩層,不矛盾亦不可互相取代
- R14(godot-specialist 2)Core Rules #14 步驟 1 新增「具體 API 的例示更正」:`get_open_error()` 只回報開檔錯誤,不回報 `store_*`/flush 失敗;明文區分開檔/寫入/flush 三類檢查機制;**flush 是否提供可檢查的失敗訊號列為 Open Questions 新增必辦查證項**(若無直接訊號須定案間接手段,不得因無回傳值就視為必然成功——那會讓「確保」在最關鍵的落盤環節退回第六輪修正前的空話)

**結構性建議(非任一專家發現,creative-director 提出,比照第三輪引入 Core Rules #16 矩陣的先例)**:完備性執行紀錄表的升級觸發條件改寫為可稽核版本 —— 見上方 U-3 裁決與 Ledger S13。

### 逐義務三件套核對紀錄(U-3 乙案新規則的第一次適用,對象為本輪自身)

*新規則要求:「任何一次修訂 Core Rules 內部義務句的輪次,須於同輪對受影響的每一條規則執行逐義務三件套核對,並將核對結果寫入該輪的 review-log 條目。」本輪修訂了 Core Rules #5/#8/#13/#14/#15 的內部義務句,故規則對本輪自身生效——**若不執行,本輪就會重演它正在修正的那個失敗**(第五輪訂下條件、第六輪觸發、無人執行)。以下為核對結果,逐**義務**而非逐規則。*

| 受影響規則 | 本輪新增/實質改寫的義務句 | (a) 矩陣列/無約束宣告 | (b) 專屬 AC | (c) 範圍聲明 |
|---|---|---|---|---|
| #5 | 路徑(四)與路徑(三)的邊界(失敗訊號形式決定路徑歸屬) | ✅ 併入既有生命週期通知列(已改為四分支並標註各路徑保證強度) | ✅ AC-73(既有,涵蓋路徑四保證前提) | ✅ 本義務**自身即為範圍聲明**(界定保證的充分條件) |
| #5 | 重複回寫失敗門檻的規則層定義(連續兩次、計數歸零條件、不持久化) | ✅ 同上列 | ✅ AC-76(既有)+ **AC-77**(新增,第一組 GIVEN 直接驗證門檻) | ✅ 已明文說明不持久化的理由(避免成為第五個需納入 #8 保護的 manifest 欄位) |
| #8 | 操作原子性(同一次序列化的同一份緩衝區) | ✅ 併入既有 #8 多列 | ⚠️ **無專屬 AC——已明文標註為已知缺口**,見下方說明 | ✅ 對 Core Rules #4 單執行緒模型的前提聲明已補上 |
| #8 | 可選讀回比對的耗時歸屬 | ✅ 同上 | ✅ AC-36(已擴充第四段子成本分桶) | ✅ 已聲明其對校準基礎的失真風險 |
| #13 | 唯讀存取可用條件擴大(兩個附加前提) | ✅ 併入既有 #13 多列 + #15 介面分類表 | ✅ **AC-77**(新增,三組 GIVEN 涵蓋開放/不開放/執行中三種情境) | ✅ 兩個附加條件缺一不可,並說明後者保留「無重入疑慮」原始論證 |
| #14 | 「確保」範圍涵蓋步驟四 | ✅ 併入既有 #14 多列 | ✅ **AC-75**(已擴大並逐步驟列舉) | ✅ 已聲明原排除的具體危險 |
| #14 | 具體檢查 API 三類區分(開檔/寫入/flush) | ✅ 同上 | ✅ AC-75(檢查行為本身) | ✅ flush 失敗訊號可得性已列 Open Questions 必辦查證項 |
| #14 | 第三類終止情境(OS 當機但電源未斷) | ✅ 同上 | ✅ AC-10a(既有,行程終止面)+ Open Questions 綁定 | ✅ 已明文更正二分法基準為「資料是否已離開 OS 快取」 |
| #15 | 甲類第四項:生命週期通知呼叫記錄 | ✅ 矩陣 #15 甲類列已補上對 #5 四條終止路徑的依賴 | ✅ **AC-78**(新增) | ✅ 「遷移呼叫順序記錄」已補上不涵蓋外部介面呼叫的範圍限定 |

**核對結果:9 條新增/改寫義務中,8 條三件套齊全,1 條有已知缺口。**

**已知缺口(明文登記,非遺漏)**:Core Rules #8「操作原子性」義務**無專屬 AC**。原因是它斷言的是實作的**內部結構**(雜湊輸入與寫入輸入是否為同一份緩衝區),而非任何可從外部觀測的行為差異——在單執行緒同步模型下,「序列化兩次但結果恰好相同」與「序列化一次」對外部完全不可區分,無法用黑箱 AC 分辨。可行的驗證手段是程式碼審查或靜態檢查,不是 AC。**處置**:不硬造一條假 AC 來把格子填綠(那正是本輪 B4 批評的假綠燈行為),改為明文登記此缺口,並於 `/create-architecture` 階段以「雜湊計算與寫入須共用同一緩衝區變數」作為程式碼審查檢查項落實。**此缺口即為新規則的第一個實證價值**——舊的規則級檢查會因 #8 早已多列多 AC 而綠燈通過,逐義務核對才讓它現形。

**creative-director 對個別專家嚴重度判定的裁決**:
- qa-lead「AC-59/AC-60 共用暫停選單範例」BLOCKING → **維持 BLOCKING**(不只是歸因力弱:登記制判準這條第六輪修法**沒有任何能驗證它的 AC**,屬三件套(b)缺漏,非測試品質問題)
- godot-specialist「AC-10a 執行方法論矛盾」BLOCKING → **維持 BLOCKING**(上調理由不在「AC-10a 不可執行」〔它可執行,只是換方法〕,而在拆分意圖於同輪被自己的紀錄表撤銷)
- systems-designer「F2-2 未涵蓋載入取消」RECOMMENDED → **維持 RECOMMENDED,但要求與 B2 同批**(呼叫方仍擁有恢復後的表面與交接責任,規則是不夠精確而非有洞;但與 B2 落在同一段文字,分批會製造第二次編輯)
- game-designer「F2-2 乙分支高亮/滑鼠不同步」RECOMMENDED → **維持 RECOMMENDED**(直接沿用使用者第十二輪對滑鼠奪權子機制的既有裁決,不得繞過該裁決另立門戶)
- narrative-director 1 BLOCKING → **維持 BLOCKING 並列為本輪最嚴重發現**(唯一動搖既有使用者裁決**前提**〔非方向〕的發現,跨三份文件,且既有 AC 對它結構性空轉)
- game-designer 1 BLOCKING → **維持 BLOCKING,重新框定**(專家框定為「缺一個管道」;實際問題是保護等級倒掛,違反文件自訂的嚴重度排序邏輯,這才是它該被當成阻擋的理由)
- performance-analyst、security-engineer 全數 RECOMMENDED(0 BLOCKING)→ **全數維持**(與 R2/R3/R4 既有降級標準一致:缺的是校準精度、診斷儀器或範圍措辭,不是正確性,失敗模式皆為安全失敗)。security-engineer 對核心攻擊面「無新的可構造靜默損毀路徑」的確認,creative-director 獨立複核同意——B7 是誠實度問題,不是資料完整性問題

**不合併的判定(交叉驗證結果)**:narrative-director 1 與 game-designer 3 表面都談可逆性,但**不得合併**——前者是硬性反例(全稱宣稱在具體構造下為假,BLOCKING),後者是軟性告知落差(規則為真但玩家體感不符,RECOMMENDED);合併會讓後者被前者的修法誤判為已解決。

**AC 總數變化**:73 → **75**(新增 AC-77 路徑〔四〕唯讀存取、AC-78 生命週期通知呼叫記錄診斷介面);其中 72 條今日可執行,3 條(AC-51、AC-61、AC-71)維持不可執行狀態並各自綁定交付閘門。`cursor-highlight-state.md` 新增 AC-61/62/63;`affinity-data-pool.md` 新增 AC-81。
...
**未隨此輪處理(明確記錄,非遺漏)**:序列化格式本身、fsync/OS 當機的硬體層級落盤查證、`DirAccess.rename()` 平台語意、型別白名單版本分域、三組 Godot API 名稱查證、`user://` 同檔案系統前提、路徑(二)觸發機制與狀態轉換模型、Steam Cloud 多檔案佈局互動、`max_supported_migration_depth` 數值上限——皆為 creative-director 第三輪起即明確裁定「在沒有引擎接觸的情況下本質上無法在 GDD 層解決」的項目,且第四輪已明文要求後續輪次不得再將其列為阻擋,本輪遵守。完備性執行紀錄表的**全表**逐義務升級(U-3 甲案)——使用者裁決本輪不投入,改由 Ledger S13 累積實證後重新評估,非遺漏。F2-3~F2-6(`/review-all-gdds` 2026-08-09 的 Warning 級發現)——連續第二輪未觸及,建議第八輪一併處理或明確裁定延後至下游系統設計時。

**creative-director 第八輪退場條件建議**:目標型,**3 位專家(qa-lead、systems-designer、narrative-director)**,只驗證三件事,不開新領域:
1. **本輪 7 項修訂本身有無製造新接縫**(既有慣例)。
2. **逐字查證本輪修訂清單的每一句宣稱**——**不接受任何「已修訂/已登記」的自我陳述,須指向具體行號並比對實際文字**。本輪兩次查獲第六輪宣稱高估覆蓋範圍(Dependencies 表格列仍寫「兩條路徑」、`systems-index.md` 的 AC-76 登記列從未存在),且與第三輪 R3-B5 的教訓同型、發生在四輪之後——此條款為本輪最重要的流程產出。
3. **B7 的通則化修法是否真的通則**——構造一個**非陣亡**的「來源永久不可寫入」情境(例如角色離隊、章節結構性封閉某來源),驗證新規則自動涵蓋,而非只涵蓋陣亡個案。

附既有退場條件:**發現任何一項屬於「新設計缺陷」而非「傳播失敗」者,即升級為完整模式**(本輪已證明此條款有效——升級後新增的四位專家貢獻了本輪僅有的兩項真設計缺陷)。若第八輪僅在這三點上通過,建議 **APPROVED**、移交 `/create-architecture`。
