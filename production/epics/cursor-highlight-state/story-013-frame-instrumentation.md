...
*以下 2 條為**本 story 自訂**的驗收條文,編號 `AC-S013-*`,不佔用 GDD 的 AC 號碼。*
*🔴 來源為 **ADR-0005 機制十五**,不是 GDD。*

- **AC-S013-a(量測的是同影格保證,不只 ≤ 1 幀)** —— *源自 ADR-0005 機制十五*:
  **GIVEN** 一次裝置權威轉移,**WHEN** 以 `diagnostic_last_authority_change_frame` 與對應高亮節點渲染屬性的翻轉影格比對,**THEN** 兩者**相等**(同一影格)。
  📌 **AC-12 要求的是 ≤ 1 幀,而機制六的定序給的是同一幀。** ADR 明文要求量測驗證**這個更強的保證** —— 只驗 ≤ 1 會讓定序退化不被發現。

- **AC-S013-b(收斂上限量測的取樣對象與採計區段)** —— *源自 ADR-0005 機制十五 F4 修訂與 R4-3 連帶*:
  **GIVEN** 一次奪權進度歸零事件,**WHEN** 檢視 `diagnostic_reclaim_progress_history` 的取樣來源,**THEN** 它取樣的是自繪節點每幀實際的 `modulate.a`(即呈現值 `_presented_alpha`),**不是** `reclaim_progress()` 判定值。**AND WHEN** 以該序列驗證 `reclaim_visual_convergence_max_frames`,**THEN** 只採計**下降區段**(相鄰兩筆遞減的連續段),上升區段不計入。
  🔴 **量錯對象會得到一個永遠通過的測試** —— 判定值依 GDD **必須**瞬間歸零,拿它去驗「收斂上限」必然通過。**ADR 原版本正是量錯了。** 反過來把上升段一併計入則是另一個方向的錯:會把「立即同步」誤判為違反上限。
...
🔴 **本 story 帶一個必須在實作時做掉的決定,ADR 刻意留給實作。**

ADR 明文:`diagnostic_reclaim_progress_history` **須有上界,建議環形緩衝,具體大小留實作**。它是逐幀累加的 `Array[float]` —— **沒有上界就是無界成長的記憶體洩漏。**

⚠️ **這不是「效能調校」,是本 story 的必要交付項之一。** 上界大小若無依據,**停下來問,不要自己選一個** —— 它必須大到能涵蓋 `reclaim_visual_convergence_max_frames` 的量測窗口。

另:全部診斷欄位為 **QA-only,下游業務邏輯不得依賴**;正式建置不應常駐啟用。

*權威來源:ADR-0005 `Performance Implications` 節與機制十五。*
