...
> **Epic**:單一游標/高亮狀態系統
> **Status**:⏸ **擱置(2026-09-07 管理者範圍裁決)** —— 本欄 2026-09-15 更正,原寫 `Ready`
> 🔴 **`Ready` 是錯的,而且它害人踩過一次**:2026-09-15 協調者讀本欄的 `Ready` 就把同批擱置的
> Story 008 派出去做了,沒有先對 `EPIC.md`。**四張擱置工作單的檔頭全部寫著 `Ready`,
> 而 epic 與 `one-year-plan.md` 第四之二節② 寫著擱置** —— 兩邊講相反的話。
> ⚠️ **擱置不等於可以開。** 這是**範圍裁決,不是排程延後**;要重啟先讀計畫檔第四之二節②,
> 確認裁決是否仍然成立。(008 已於 2026-09-15 經管理者重新裁決解除,理由是它變成
> 「畫面上真的打得到卡牌」的必要前置 —— **那是一次新裁決,不是本欄自動失效**。)
> **Layer**:Core
> **Type**:Logic
> **Estimate**:S(約 2–3 小時)
...
## 🔴 開工前查核(2026-09-04,協調者)—— 3 條驗收標準,有 2 條在檢查一張不存在的表

**AC-46 與 AC-46b 驗的是 `mouse_reclaim_threshold_px_by_surface_type` 這張門檻表。**
實測:`src/` 與 `tests/` 全庫搜尋 `mouse_reclaim_threshold_px_by_surface_type` 與
`reclaim_threshold_px`,**兩個字串皆零命中** —— 那張表不存在。

它屬於 **Story 014**(見該張 Implementation Notes 第 2、4 點:「門檻是逐表面類型的像素常數」、
「常數配置為 0 或負值時系統必須拒絕啟動(AC-46)」),而 014 是使用者 2026-08-11 裁決凍結的子機制。

| AC | 狀況 |
|---|---|
| AC-26 | ✅ 驗得到 —— 真的呼叫 `InputMap.get_actions()` / `action_get_events()` 掃描執行期綁定 |
| **AC-46 / AC-46b** | 🔴 **定義域是空的。** 沒有表可以驗。寫一個「表若存在則檢查」的驗證器,測試會綠而什麼都沒驗到 |

🔴 **2026-09-07 更正:本節原本的裁決題目,前提是錯的。**

原文寫「本張是否連帶建立那張門檻表(**等於踏進凍結區**)」。實際查證 2026-08-11 凍結裁決原文
(`design/gdd/cursor-highlight-state.md:347`):凍的是**硬性閘門降級**、**候選修法停止投入**、
**子機制重新設計暫停**(待手把硬體)——**沒有一句禁止實作**。而那張表本來就有工作單擁有它:
`story-014-mouse-reclaim-frozen.md`,標題即「**照現況實作,不修缺陷**」,狀態 `Ready`、前置(001/002)皆已完成。

**因此「建那張表」從來不是踏進凍結區,而是執行一張本來就可以開的工作單。**
真正該避免的是**由本張順手把表建出來** —— 那會讓同一份交付有兩個擁有者。

✅ **管理者 2026-09-07 裁決:先做 Story 014。** 該表已於同日建立為
`src/ui/cursor/mouse_reclaim_threshold_config.gd`(`MouseReclaimThresholdConfig`,四種表面類型各一欄位,
數值為**暫行值**,GDD 明文待垂直切片校準)。**AC-46 / AC-46b 的定義域因此不再是空集合。**

⚠️ **但本張仍未開工,而且 AC-46/46b 的實作責任在本張,不在 014** ——
`MouseReclaimThresholdConfig` 裡的兩個 `assert` 是除錯期防呆,`assert` 可能在發行版建置被移除,
**不構成 AC-46/46b 要求的「系統於初始化階段偵測並明確處理」**。本張要建的
`CursorStartupValidator`(ADR-0005 機制七)才是那個驗證器。

⚠️ **這是 Story 012 被判 Blocked 的同一個形狀,較輕的版本** —— 012 是全部驗收標準的定義域皆為空集合,
本張是 3 條裡的 2 條。**同型問題應該有同型的處置,不該因為「只有三分之二」就默默做下去。**
