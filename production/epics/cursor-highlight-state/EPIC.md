...
**兩張都只差 005**,故 005 是本 epic 目前唯一能解鎖後續的一張。
**012 Blocked**(驗收標準的定義域是空集合,需先裁決強制執行落點)。

> 🔴 **2026-09-07 更正三處**:本段原寫「下一波可開:006;011 因 010 完成而解鎖」與「014 凍結」,
> 而 **011 與 014 兩張皆已於 2026-09-07 完成**。014 的「凍結」是**凍結重新設計**,
> 從來沒有凍結實作 —— 那次誤讀讓這張工作單多等了 26 天,經過寫進兩份工作單與管理者狀態頁,
> 直到有人回頭讀 `design/gdd/cursor-highlight-state.md` 的裁決原文才發現。
> **教訓不是「凍結欄位要寫清楚」,是「轉述一條裁決之前先讀它的原文」。**
**012 Blocked**(驗收標準的定義域是空集合,需先裁決強制執行落點);**014 凍結**。

### 🔴 2026-09-04 開工前查核:「前置滿足」不等於「驗得到」

盤點下一波時逐張查了「這張要驗的東西現在存不存在」,結果與上一行的「皆可開」不一致。
**三張裡只有 009 的驗收對象是完整的**,另兩張各自缺一塊 —— **全文寫在各該工作單的
「開工前查核」節,本節只記結論:**

| Story | 驗收對象查核結果 |
|---|---|
| **009** | ✅ 四條 AC 全部驗得到。呼叫方由測試替身扮演,AC-62 原文明文允許 |
| **006** | 🔴 3 條 AC 有 **2 條**(AC-46 / AC-46b)在檢查 `mouse_reclaim_threshold_px_by_surface_type` 這張表,**全庫零命中** —— 它屬凍結的 Story 014 |
| **011** | 🔴 8 條 AC 有 **3 條**(AC-36 / AC-48 / AC-49)要比較的三種視覺**沒有任何圖形規格**;另 **AC-31b** 執行期定義域為空、**AC-31** 無條件成立 —— 皆因 `reclaim_progress()` 在 `_reclaim == null` 時**恆回傳 0.0**。<br>**連帶後果**:機制十三的自繪載體每幀 `modulate.a = 進度`,故**執行期永遠 100% 透明、不會被畫出來**,而本張綁著「必須真的開視窗用眼睛看」的硬性義務 |

✅ **2026-09-07:上表三列已全部關閉,但關閉方式各不相同 —— 差別值得記住。**

| Story | 當時的缺口 | 怎麼關的 |
|---|---|---|
| **009** | (無) | 2026-09-04 直接完成 |
| **006** | 門檻表全庫零命中 | **缺口被別的工作單填掉了** —— Story 014 於 2026-09-07 建出 `MouseReclaimThresholdConfig`,AC-46/46b 現在有東西可查。⚠️ **但該檔的除錯期 `assert` 不等於 AC-46/46b** —— 那要的是載入期系統級拒絕,仍是 006 自己要寫的 `CursorStartupValidator` |
| **011** | 3 條 AC 沒有圖形規格;AC-31/31b 定義域為空 | **一半靠補前置、一半靠誠實登記** —— 定義域是 014 建好策略後才有的;3 條缺圖形規格的 AC 則**沒有被解決**,是依其原文本身的 ADVISORY 等級登記為待美術規格,不是驗過了 |

🔴 **後者是本輪最該記住的一件事**:「開工前查核發現缺口」有兩種完全不同的出路 ——
**補齊它**,或**在文件裡寫清楚它沒被補齊**。兩者都可以接受,**但混在一起講就會讓
第二種看起來像第一種。** 011 的結案紀錄逐條列出哪 3 條屬第二種、以及誰擁有它們。

📌 **通則(值得記住的形狀)**:`/story-readiness` 檢查的是「前置 story 完成了沒」與
「ADR 核准了沒」,**它結構上不會檢查「這張要驗的東西是否存在」**。
本 epic 已有 012 一張因此被判 Blocked;**006 與 011 是同一個形狀的較輕版本,
不該因為「只缺一部分」就默默做下去。** 兩張開工前皆需先取得裁決,各自的裁決題目寫在該張。
