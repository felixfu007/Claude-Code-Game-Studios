...
3. **依 U-004 探針結果決定最終按鍵**——若探針發現任何候選鍵有衝突,以探針報告為準調整,
   ...
   ### 🔴 U-004 探針已於 2026-09-16 執行完畢,結果如下 —— **不得再沿用「建議值」**

   **報告全文與原始 log**:`prototypes/u004-inputmap-probe-2026-09-16/README.md`
   (等級 (A),執行檔 `probe_input_map.gd`,探針自陳重新實作的規則與未計入因素皆已逐條揭露)。

   | 動作 | 鍵盤 | 鍵盤結論 | 手把 | 手把結論 |
   |---|---|---|---|---|
   | `battle_cancel` | Esc | 🔴 已被 4 個內建動作佔用 | B | 僅 `battle_end_phase`(刻意的過渡狀態) |
   | `open_hand` | C | ✅ 未佔用 | X | 🔴 **已被 `ui_colorpicker_delete_preset` 佔用** |
   | `next_target` | Tab | 🔴 已被 3 個內建動作佔用 | RB | ✅ 未佔用 |
   | `prev_target` | Shift+Tab | 🔴 已被 3 個內建動作佔用 | LB | ✅ 未佔用 |
   | `battle_menu` | M | ✅ 未佔用 | Start | ✅ 未佔用 |

   🔴 **探針推翻了 EPIC.md 陷阱九轉錄的那句話的字面版本。** 兩份 UX 規格建議手把用 **X**,
   理由寫的是「為了避開已知的那一個」—— 而實測 **X 同樣被一個雙側(鍵盤 Delete + 手把 X)
   內建動作 `ui_colorpicker_delete_preset` 佔用,不是零佔用**。

   ⚠️ **但方向性結論仍然成立(X 優於 Y),理由換了**:Y 的衝突對象 `ui_select` 鍵盤側是
   Space,與本專案**現有**的 `battle_confirm` 直接重疊,且其觸發前提(清單類原生元件取得焦點)
   與 `battle-menu.md` 明文允許的原生 focus 有真實交集;而 X 的衝突對象觸發前提是 ColorPicker
   元件,本專案設計文件裡完全沒有這個需求。
   🔴 **這一段是判斷不是量測**,探針作者已明文標示,`README.md` 有完整推理。

   📌 **對 U-005 的實際約束**:寫 `project.godot` 時**不得寫下「X 未被佔用」這種話**。
   正確措辭是「**X 的衝突對象與本專案介面元件類別無重疊**」。前者是假的,後者是探針支持的。
