...
> **型別**:Integration
> **狀態**:✅ **Complete(2026-09-24,管理者裁決逐字「判完成」)**
>
> 📌 **本張型別是 Integration(BLOCKING)** —— 證據關卡就是自動化測試本身,
> 不另需書面證據檔。5 條測試全過(story `Test Evidence` 列的 5 條預期測試名全部照抄寫入)。
> 全套實測 **956 條、0 errors、1 failures(既有已核准的刻意紅燈)、0 orphans、76/76 套件**,
> 由協調者獨立重跑核對,非採信回報。
>
> 🔴 **一併登記的缺口(與 U-013、U-014 相同形狀,連續第三張)**:
> **本批零截圖,沒有任何人看過這個畫面。** 成因是曝光約束(座位旁有人經過),全程 headless。
> **具體未知項**:強制棄牌提示文字的版面位置為**實作者自陳的佔位品質**
> (沿用 S5 收合態的寬度公式,套用在 S4 展開態上,位置未必精確)。
>
> ⚠️ **其餘誠實登記的缺口**:未寫常駐 `test_sensitivity_proof_*`;
> 「卡片 session 跨回合邊界仍開啟」的邊界情況未測(失效方向為 fail-closed)。
>
> ✅ **實作中順手修掉一個既有真實 bug**:`hand_bar.gd` 的 `_rebuild_slots()` 迴圈邊界
> 改為 `maxi(_max_slots, _slot_kinds.size())` —— 原本**手牌達 6 張時第 6 張沒有面板、無法被選取**。
> 🔴 **那不是巧合**:強制棄牌唯一的觸發情境就是手牌剛好 6 張,
> 若該 bug 復發,玩家會被迫棄牌卻有一張選不到,**直接卡死**。
> **管理者裁決「補一條測試」釘住它**(2026-09-24)。
> **估時**:M
