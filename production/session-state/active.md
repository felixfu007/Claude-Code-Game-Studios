...
## ✅ 第十七批管理者裁決:游標裝置衝突 = 滑鼠優先(2026-08-27)
...
🔴 **但協調者已就地提出一個裁決未涵蓋的判定條件,尚未取得管理者答覆**:

**「一律滑鼠優先」在滑鼠靜止時會讓手把完全失效。** 具體情境:玩家用滑鼠玩了一陣子,
放下滑鼠、拿起手把 —— 滑鼠一動也沒動,但主權還在它手上,於是**推蘿蔔游標不動,
玩家會以為手把壞了**。

**協調者建議的最小補充**:改成「**誰最後有動作,誰拿主權**」—— 滑鼠**移動時**取得主權,
手把**有輸入時**取得主權。⚠️ **這個補充與管理者的理由完全同向**:他說的「不會有人同時用兩個」
正是「最後動作者優先」成立的前提;而「一律滑鼠優先」只在兩者同時有輸入時才需要,
那個情境依他的理由本來就不存在。

**狀態:待管理者確認。** 在確認之前,**接線那一批不得寫死「滑鼠恆有主權」** ——
把裝置主權抽成一個可替換的判定點即可,兩種讀法的實作差異只有幾行。
