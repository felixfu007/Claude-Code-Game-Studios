...
<!-- STATUS -->
Epic: 🟢 **專案想像重新校準完成 —— 23 項裁決已落地**,遊戲的身分、關係模型、結局架構、世界觀骨架、對立路線的存在理由皆已定案
Feature: ✅ **結局系統設計文件已落檔**(`design/gdd/endings.md`,310 行)。**目前沒有阻擋性待裁決項。**
Task: 🟡 **2026-08-26 第十一批。** 管理者裁決 D19~D23,關閉本專案最大的已知漏洞(R2 平均分攤),並回答一個打中整條對立路線的質疑。

🔴 **接手第一件事:讀 `design/gdd/gameplay-flow-decisions.md`(約 1250 行)。** 它是裁決的權威記錄,含 D1~D23、兩項管理者澄清(C1/C2)、兩軸模型、五輪專家覆核。**本節只是索引,不要只讀本節就動手。**

**權威檔案四份**:
- `design/gdd/gameplay-flow-draft.md` —— 管理者原話逐字保存(**任何情況下不改**)
- `design/gdd/gameplay-flow-decisions.md` —— 23 項裁決與待決清單(**主檔**)
- `design/gdd/endings.md` —— ✅ **2026-08-26 新落檔**,結局系統設計文件
- `docs/reviews/gameplay-flow-gap-analysis-2026-08-25.md` —— 七方落差審核

## 這款遊戲現在是什麼(一句話)

**戰鬥裡沒有任何一件事是你算不出來的。算不出來的是人——而祂知道這一點。**(D1)

## 已定案的核心模型

- **兩軸**:關係軸(親近←→對立,合作卡/**對立卡**推動,決定戰術) + 腐蝕軸(只有低語卡推動,決定結局)
- ⚠️ **命名已於 2026-08-26 更改(D23)**:`競爭卡`→`對立卡`、`競爭結局`→`對立結局`。
  **只往後生效,歷史段落保留舊詞,D23 是權威對照表。** 已核准的 ADR/登記表零出現,無須改動。
- **對立的存在理由(D22)**:**拒絕互相掩護** ——「我看到你這裡會出事,但我不會幫你蓋過去」。
  🔴 **這不是冷漠,是高強度的注視。任何違反此點的後續文本都是寫錯了。**
- **對立不是壞事**(D5);腐蝕**不可修復**(D13)
- **結局兩段式**(D14):先判腐蝕**總和**(D19),達標→壞結局;未達標→好感度偏向決定合作/對立結局
- **三結局平起平坐,無位階**(D20,已廢除「真結局/普通結局」的說法)
- **沒有永久死亡**(C2):角色倒下=關卡失敗=回到上一次營火;**全部回滾**(D7)、**連營火一起重做**(D8)
- **營火=檢查點**;手動存檔只在營火且覆寫同一份進度(D12,非時光機)
- **祂的動機**(D15):力量不稀有,有趣才稀有。咒印是甜點,主餐是看五個人互相撕咬
- **從頭到尾都是奈亞在搞鬼**(D16)
...
| 原本開著的 | 由什麼關閉 |
|---|---|
| 🔴 **R2 平均分攤退化玩法**(本專案最大已知漏洞) | **D19**(改讀總和,分散不減少總量) |
| 🔴 D14 待確認二:腐蝕達標是誰達標 | **D19** |
| 🔴 D14 待確認一:結局命名對應 | **D20** |
| 🔴 D18 推出的「跨線後六關是死路」 | **D19 + D10**(總量把撞線點自然拉後;D10 本就不說是哪條路) |
| 🔴 管理者質疑:五個陌生人為何有心情競爭 | **D22 + D23**(機制全保留,只改兩個名詞) |
| 🟡 結局輪廓不能落檔 | ✅ **已落檔** `design/gdd/endings.md` |

## 🟡 待裁決(皆不阻擋開工)

1. **咒印彩蛋** —— 兩個乾淨結局最後一格畫面咒印仍在他手上(彙整者提出,未經任何人裁決)
2. **D17 腐蝕殘留呈現是否也適用對立結局**(結局文件已類推適用,未經明確確認)
3. **對立結局第十關是否與合作結局共用關卡** —— 需 `game-designer`/`level-designer` 確認工作量
4. **範例卡名** —— 原文「我們來比賽」與新 register 不符,需 `writer`/`game-designer` 重新命名
5. **腐蝕門檻數值** —— D18 只給方向「很低」,待可玩版本量測
6. 其餘見決策檔第三節「待決」(Q1~Q16,部分已關閉)

## 🔴 仍未解決的結構性問題(兩個,同一模式)

1. **存檔的觸發者不在 MVP**:C2 使營火=檢查點,存檔成核心迴圈必需品,但呼叫存檔的「章節/戰役結構」系統於 2026-08-25 被降級為垂直切片、Not Started
2. **揭露義務的承接者不在 MVP**:跨腐蝕門檻時必須告知「有一條路關上了」,承接系統是「敘事解鎖與結局分支系統」,同樣是垂直切片、Not Started

## 現在就能動工

🟢 **好感度資料層寫成程式** —— 唯一能換到實作速度數據的工作,重要性最高
🟢 **五名主角基礎立繪** —— ⚠️ 腐蝕變體階數待門檻定值,先畫基礎組
🟢 **五名角色命名** —— 編劇已指出角色連名字都沒有,台詞只能用代號

## ⚠️ 給接手者的紀律(實測學到的)

1. **不得用行號指向任何文件** —— 已全面改為穩定指法(章節標題、裁決編號)
2. **專家報告的定位與全稱宣稱不可直接轉抄** —— 2026-08-26 專家宣稱「競爭卡在已核准文件零出現」,彙整者自行 grep 查證後才採用(結論正確,但 ADR-0002 有 5 處同字不同義的「執行緒競爭」)
3. **獨立審核防得了互相污染,防不了共同誤讀** —— 唯一能防的是回頭問原作者
4. **彙整者的推理在寫進文件前必須查證** —— 2026-08-26 三次被專家修正:兩次寫歪裁決、一次擅自擴大問題範圍

⚠️ **`.gd` 檔依檔案路由表歸 `godot-gdscript-specialist`;劇情/世界觀歸 narrative-director、台詞歸 writer。** 環境規則要求使用者未要求時不得呼叫 Agent 工具,**開始前必須先向使用者提出此衝突。** 本批授權來自使用者逐次明確指示,**不自動延伸到下一批。**
<!-- /STATUS -->

**最後更新**:2026-08-25(**第六批**)—— ADR-0002 與 ADR-0003 兩份都跑完完整強度的覆核並修畢落檔,**五個核准條件備齊,只差管理者按下核准**。提交 `cbe18e3`/`48402bb`/`d933e1c`,全部已推送。**詳見檔末「第六批」一節。**
