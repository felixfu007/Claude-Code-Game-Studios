...
Feature: 垂直切片《迷霧岔口》。**棋盤、移動範圍、視線遮蔽完成**,下一步是單位與回合
Task: 🟢 **2026-08-26 第十三批。25 個測試全過、0 孤兒、exit 0,並在全新複本上獨立複驗過。**
...
| **測試** | ✅ **25 個(原 9 + 棋盤 8 + 視線 8),0 失敗、0 孤兒、exit 0** |
...
1. 🔵 **單位資料層** —— 五名主角 + 5 敵人,HP/ATK/DEF、武器分層 `(min_range, max_range)`、MP
2. 🔵 回合順序與行動旗標(移動/攻擊各一次,可暫緩)
3. 🔵 攻擊合法性 —— 曼哈頓距離落在 `[min_range, max_range]` **且**射程 ≥2 時通過 `LineOfSight.is_clear()`
4. 🔵 傷害結算 `max(0, ATK − DEF + Φ)` —— **零隨機,專案級禁令**
5. 🔵 最小的「這兩人現在給你 +N」提示(**不做整套好感度 UI**)
6. 🔵 玩一次,量 `affinity-position-provisional.md` 第 5 節那五項
...
**最後更新**:2026-08-26(**第十三批**)—— **專案第一段遊戲邏輯落地**:棋盤資料層 + 可達格 Dijkstra + 視線遮蔽,三個新程式檔、一個地圖資料檔、兩個測試檔。測試 9 → 25 全過、0 孤兒、exit 0,並在暫存目錄的全新 clone 上獨立複驗。派工方式改為「兩張約 300 字的短派工單平行派工」,一次成功。
