...
Feature: 垂直切片《迷霧岔口》。**規則層全部完成,畫面層與互動控制器兩個零件也已完成 —— 但尚未接線,還不能玩**
Task: 🟡 **2026-08-27 第十七批。`BoardView`(畫得出來)+ `BattleController`(判斷得出來)兩包平行落地,各自有測試。129 個測試全過、0 孤兒、exit 0,主 session 獨立複驗 + 交叉驗證 + 親手打斷閘門證明測試會失敗而非掛住。🔴 下一步是接線 —— 滑鼠點下去目前接不到任何東西。**

## 🔴 接手第一件事

**下一件事:把滑鼠接到控制器上。** 第十七批把「畫面」與「操作邏輯」兩個零件**都做好了,
但兩者之間沒有接線** —— `BoardView` 會畫,`BattleController` 會判斷,可是**滑鼠點下去那一下,
目前沒有任何程式碼接得到**。⚠️ **所以現在仍然不能玩。**

**做這件事要讀的**:

1. `src/gameplay/battle/battle_controller.gd`(524 行)—— **唯一要呼叫的入口是 `click_tile(pos)`**,
   回傳 `{"action": ...}` 五種之一。完整 `##` 註解,**讀類別層註解就知道裁決優先序,不必逐行讀**
2. `src/ui/battle/board_coords.gd`(111 行)—— `window_to_grid(window_pos, window_to_canvas, origin)`。
   🔴 **`window_to_canvas` 一定要向引擎查 `Window.get_final_transform().affine_inverse()`,不可自己用視窗尺寸推算**
3. `src/ui/battle/board_view.gd`(214 行)—— `render_terrain()` / `render_pieces()` /
   `set_move_highlights()` / `set_attack_highlights()` / `set_cursor()` / `clear_cursor()`
4. 本檔「🔴 五個會咬人的實測事實」整節(5 項,寫程式前必看),以及「🟡 本批新產生的規格問題」
   整節(現在 20 項,第 15~20 項是本批新增)

**接線要注意的四件事**:

- 🔴 **`BoardView` 不得持有 `BattleState`,`BattleController` 不得持有任何節點。** 這條界線是刻意的;
  接線層(新檔,建議 `src/ui/battle/battle_screen.gd`)是唯一同時認識兩邊的東西
- 🔴 **輸入一律走 `_input()`;不得用 `_unhandled_input()` 做確認鍵或裝置判定;不得讀 `InputEvent.device`。**
  這是 ADR-0005 已登記的禁令,現在遵守成本近乎零,違反了以後接不上
- 🔴 **必須同時支援鍵盤/手把游標移動。** `technical-preferences.md`:主機沒有滑鼠指標,
  不得有 hover-only 互動。裝置衝突規則已由管理者裁決,見下方「第十七批管理者裁決」
- ⚠️ **`prototypes/board-render-input-spike-2026-08-27/` 是拋棄式的,`src/` 不得引用。**
  它的結論已經正式化進 `board_coords.gd`

**寫程式不需要讀的**:所有 ADR、所有 `/architecture-review` 報告、劇情裁決三份
(`characters.md` / `gameplay-flow-decisions.md` / `endings.md`)。
...
| **測試** | ✅ **129 個(原 97 + 控制器 25 + 座標 7),0 失敗、0 錯誤、0 孤兒、exit 0。主 session 獨立複驗全套** |
...
8. 🔵 **接線**(`src/ui/battle/battle_screen.gd`,新檔)—— **下一個要做的**。歸 `godot-specialist`。
   詳見本檔開頭「接手第一件事」的四項注意事項
9. 🔵 最小的「這兩人現在給你 +N」提示(**不做整套好感度 UI**)。⚠️ 送入管道已存在:
   `BattleController` 建構子第三參數 `phi_provider`
10. 🔵 玩一次(真人),重測對局長度與丙的存活率 —— **自動對打的數字不能取代這一步**
