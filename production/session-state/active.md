...
Task: 🟢 **2026-09-01 第二十六批完成:ADR-0001 與 ADR-0005 已 `Accepted`(管理者裁決)。五份 ADR 中四份已核准,只剩不在 12 個月範圍內的 ADR-0004。** 過程:`TD-ADR` 架構總監覆核**首次對這兩份執行**(自專案成立從未跑過),兩份皆 CONCERNS → 修訂 → 窄範圍複驗(ADR-0001 那輪**判不通過**、抓到三處主 session 寫錯的事實,更正後放行);引擎探針 0001 四支全過、0005 六項關五項。**解鎖 #3 游標/高亮系統可正式開工 —— 四個已核准系統中第一個。** 🔴 **#4 戰棋仍不能開工**,另受其自身 OQ-2(我方基準數值表無人擁有)阻擋。**兩條硬性實作義務隨核准生效:游標圖層獨佔 `CanvasLayer`、機制四之二自行過濾 `InputEventKey.echo`。**
<!-- /STATUS -->

## 🔴 接手第一件事(2026-09-01 第二十五批寫入 —— 取代下方第二十四批的指示順位)
