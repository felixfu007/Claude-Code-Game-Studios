...
<!-- STATUS -->
Epic: 架構階段收尾 → **實作階段起點**
Feature: ✅ MVP 範疇已裁決收斂(12 → 9 個系統)—— 下一個焦點是**寫出第一行遊戲程式碼**
Task: 🟢 **2026-08-25 第八批。管理者裁決 MVP 範疇以 `game-concept.md` 為權威,`systems-index.md` 已重新標記。** #2 存檔、#7 章節/戰役結構、#12 教學系統自 MVP 降級至垂直切片;#6 只做好感度對話卡牌、#10 只做預判標記。連帶修掉 Progress Tracker 的 6 個過期數字。

**MVP 施作順序(Order 欄未重排,遇 `Vertical Slice` 跳過)**:1 → 3 → 4 → 5 → 6 → 8 → 9 → 10 → 11。

⚠️ **兩個由裁決推導出的洞已就地寫入 `systems-index.md` 對應列,設計那兩個系統前必讀**:
(一)#8 支援對話系統的上游 #7 被降級 —— MVP 用寫死觸發點,**不得去等 #7 定案**;
(二)#12 降級的是引導系統這套程式,**MVP 仍需一張手工教學關卡**,否則驗收協議 1 無法執行。

⚠️ **不要再往 #2 / #7 / #12 投入設計或架構工作。** ADR-0004(存檔原子寫入)維持 `Proposed` 即可 —— 它不在通往可玩版本的最短路徑上(第七批把它排在第 2 順位是錯的)。

🔴 **下一步唯一該做的事:好感度資料層寫成程式**(唯一已解除阻擋者),並建立目前不存在的 `project.godot`。**它的第一個交付是真實的實作速度數據** —— 在那之前所有時程估計的實作月數都是猜的。

🎨 **同批第三~五個裁決(2026-08-25)**:美術走**像素風、自己畫**;流程劑量採**建議上限**
(設計文件 ≤ 400 行、審查 ≤ 2 輪、ADR 只在跨系統契約時寫、同一份不改第三次;**只往後生效** ——
已寫進 `.claude/docs/technical-preferences.md`,因為計畫檔不會自動載入);
**像素風硬參數定案為 480×270 套裝**,全文 `design/art/art-direction.md`(115 行,新建)。

✅ **畫圖現在可以開始** —— 硬參數已定,且美術圖檔不受介面分層決定影響(實測:換容器不影響 PNG)。

⚠️ **我先前寫的「貼圖濾波是建立時必須設對的不可逆項」是錯的,已於本批更正三處。**
實測:貼圖濾波是**最可逆**的設定(不寫入 `.import` 檔,改單一全域設定即可,無須重新匯入)。
**真正不可逆的只有基礎解析度** —— `art-director` 與 `godot-specialist` 獨立得出同一結論。

🔴 **`stretch/mode` 絕不可設 `"viewport"`**(會把介面壓進低解析度、文字模糊)。
分層須用 `SubViewport` + `SubViewportContainer.stretch_shrink`,細節與來源等級見
`technical-preferences.md`「美術方向與像素風專案設定」節的三項實測事實。

⚠️ **ADR-0005(游標,1609 行)有新的已知待修**:分層後存在兩套座標系。**因此不該凍結。**

**第 8 個月停損點仍未獲明文同意** —— 三項裁決中唯一還空著的。

**接手先讀檔末「第八批」、「同批第三、四個裁決」與「專家覆核輪」三節。**
