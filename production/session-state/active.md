...
<!-- STATUS -->
Epic: 🟢 **專案想像重新校準 —— 18 項裁決已落地**,遊戲的身分、關係模型、結局架構、世界觀骨架皆已定案
Feature: 結局輪廓待敘事總監第二輪覆核;**三項機制交互(腐蝕門檻高低 × 誰達標 × 跨線後怎麼辦)待管理者裁決**
Task: 🟡 **2026-08-25 第十批。管理者提供遊戲流程雛形(克蘇魯+戰棋+卡牌、十關、三結局),七方獨立落差審核後逐項討論,產出 18 項裁決。**

🔴 **接手第一件事:讀 `design/gdd/gameplay-flow-decisions.md`(1029 行)。** 它是本批唯一的權威記錄,含 D1~D18 全部裁決、兩項管理者澄清(C1/C2)、兩軸模型、四輪專家覆核結果(R1~R8)、以及待決清單。**本節只是索引,不要只讀本節就動手。**

**權威檔案三份**:
- `design/gdd/gameplay-flow-draft.md` —— 管理者原話逐字保存 + 檔末澄清節
- `design/gdd/gameplay-flow-decisions.md` —— 18 項裁決與待決清單(**主檔**)
...
- **兩軸**:關係軸(親近←→對立,合作卡/競爭卡推動,決定戰術) + 腐蝕軸(只有低語卡推動,決定結局)
- **對立不是壞事**(D5);腐蝕**不可修復**(D13,「就像人與人心中的芥蒂」)
- **結局兩段式**(D14):先判腐蝕,達標→壞結局(好感度完全不看);未達標→好感度**偏向**決定合作結局/競爭結局
- **沒有永久死亡**(C2):角色倒下=關卡失敗=回到上一次營火;**全部回滾**(D7)、**連營火一起重做**(D8)
- **營火=檢查點**;手動存檔只在營火且覆寫同一份進度(D12,非時光機)
- **祂的動機**(D15):力量不稀有,有趣才稀有。咒印是甜點,主餐是看五個人互相撕咬
- **從頭到尾都是奈亞在搞鬼**(D16):森林是祂、五人是被帶進去的、中期五關的怪物就是角色的心魔

## 🔴 待管理者裁決(依急迫度)

1. 🔴 **「腐蝕達標」是誰達標** —— 敘事總監建議**五人總量**(理由見決策檔「一之三」節 A)。**這一項拍板後,R2 平均分攤漏洞當場消失。**
2. ✅ **敘事總監第二輪已完成**,見決策檔「一之三」節。建議:「腐蝕達標」＝**五人總量**,跨線通知維持 D10 不加碼。**待管理者拍板。**
3. 其餘見決策檔第三節「待決」(Q1~Q16,部分已關閉)

## 🔴 已知未解的最大風險

**R2 平均分攤退化玩法**:門檻逐角色、低語卡每次弄髒 2 人,**專家實算:集中打法第 8 次破門檻,平均分攤約 17 次**。原以為的抵銷力量(D6 開門效應)**方向是反的**。門檻越低,相對收益越大。**尚未解決。**

## 🔴 兩個「必需系統不在 MVP」的對撞(同一模式,兩個實例)
...
🟢 **好感度資料層寫成程式** —— 在本批之下重要性上升,且是唯一能換到實作速度數據的工作
🟢 **五名主角立繪** —— 任何版本都要。⚠️ 但腐蝕變體立繪的階數取決於 D18 門檻定值,**先畫基礎組**

## ⚠️ 給接手者的三條紀律(本批實測學到的)

1. **不得用行號指向任何文件** —— 本批已把兩份新檔約 100 處行號全部換成穩定指法
2. **專家報告的定位不可直接轉抄** —— 專家從 grep 片段定位,行號未必對;**彙整者必須自己開檔核對**(本批抓到三處錯)
3. **獨立審核防得了互相污染,防不了共同誤讀** —— 七位專家各自獨立卻全體誤讀同一句原話;**唯一能防的是回頭問原作者**

⚠️ **`.gd` 檔依檔案路由表歸 `godot-gdscript-specialist`;劇情/世界觀歸 narrative-director、台詞歸 writer。** 環境規則要求使用者未要求時不得呼叫 Agent 工具,**開始前必須先向使用者提出此衝突,不可靜默處理。** 本批授權來自使用者逐次明確指示,**不自動延伸到下一批。**
<!-- /STATUS -->
