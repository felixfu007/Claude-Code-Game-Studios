...
<!-- STATUS -->
Epic: 🟢 **《盲目於微光》—— 專案已從「純文件」進入「有程式在跑」**
Feature: 垂直切片(單一戰棋關卡)。基礎設施完成,**下一步是寫棋盤程式**
Task: 🟢 **2026-08-26 第十二批。專案首次有東西能執行、測試首次真的跑過(9 個全過、exit code 0)。**

## 🔴 接手第一件事

**這一批是「開始寫程式」的批次,不是劇情批次。** 要寫程式,讀這三份:
...
**劇情/世界觀的 23 項裁決在 `design/gdd/gameplay-flow-decisions.md` 與 `design/gdd/endings.md`,
寫棋盤程式時不需要讀。**
...
| | 狀態 |
|---|---|
| `project.godot` | ✅ 480×270、`canvas_items`、整數縮放、Nearest 濾波。**每個鍵都實機驗證,零訓練資料推測** |
| `src/` 骨架 | ✅ `core`/`gameplay`/`ai`/`ui`/`tools`,各附說明檔。**`networking` 依專案級禁令不建立** |
| `src/ui/GameRoot.tscn` | ✅ 世界層/介面層分層,`texture_filter` 已手動覆寫為 Nearest |
| 佔位美術 | ✅ `assets/art/placeholder/` 12 張色塊。生成腳本在 `tools/asset-pipeline/` |
| **測試** | ✅ **GdUnit4 v6.2.1,9 個測試全過、0 孤兒、exit code 0** |
...
2. **不可手動設 `SubViewport.size`** —— 引擎**直接拒絕賦值並印警告**,不是「下一幀被覆寫」。
   (2026-08-26 修正:參考文件原本的機制描述是錯的,會讓人除錯時往錯方向找。)
3. 🔴 **GdUnit4 有假綠燈陷阱** —— 它丟棄含 `GdUnitCmdTool.gd` 那個 token 之前的全部參數;
   少了那個 token 會**靜默退化成印說明文字並 exit 0**,測試完全沒跑卻是綠燈。
   **exit code 定義**:`0` 成功、`100` 有錯誤/失敗、`101` 僅有孤兒節點、`103` headless 未加旗標。
   **101 不是失敗,是「通過但漏了節點」。正解是修漏,絕不是把 101 當成功。**

## 下一步(依順序)

1. 🔵 **寫棋盤** —— 13×6 格、地形移動成本(1/2/3)、視線遮蔽。⚠️ **`.gd` 檔歸 `godot-gdscript-specialist`**
2. 🔵 單位、移動力、回合順序
3. 🔵 傷害結算 `max(0, ATK − DEF + Φ)` —— **零隨機,專案級禁令**
4. 🔵 最小的「這兩人現在給你 +N」提示(**不做整套好感度 UI**)
5. 🔵 玩一次,量 `affinity-position-provisional.md` 第 5 節那五項
...
4. 對立結局第十關是否與合作結局共用關卡(工作量問題,需 `game-designer`/`level-designer`)
5. 範例卡名(取代原文「我們來比賽」)
...
1. **`Φ` 的來源系統沒有設計文件** —— 「好感度—位置連鎖系統」`Not Started`、規模 L。
   切片用 `design/quick-specs/affinity-position-provisional.md` 的暫定值繞過,**由切片實測收斂**。
2. **存檔的觸發者不在 MVP**(章節/戰役結構已降級為垂直切片、Not Started)
3. **揭露義務的承接者不在 MVP**(敘事解鎖與結局分支系統,同上)

## ⚠️ 派工紀律(2026-08-26 實測學到,代價昂貴)

**四次派工,前三次連續空轉:約 100 次工具操作、22 萬 token、零產出。**
**根因:派工單太長。** 為了防止 Agent 憑記憶寫錯 API,塞了大量背景、禁令與引用 ——
**結果它們在讀完之前就把預算燒光。防止出錯的資訊擠掉了做事的空間。**

**第四次把派工單從三千多字砍到不到三百字,當場成功。** 保留的只有:
能跑的指令一行、關鍵數值的定義、要做的事、四條限制。
🔴 **給答案,不要給警告。** 警告要 Agent 自己去找,答案讓它直接用。

⚠️ **`.gd` 檔依檔案路由表歸 `godot-gdscript-specialist`;劇情歸 narrative-director、台詞歸 writer。**
環境規則要求使用者未要求時不得呼叫 Agent 工具,**開始前必須先向使用者提出此衝突。**
本批授權來自使用者逐次明確指示,**不自動延伸到下一批。**
<!-- /STATUS -->
