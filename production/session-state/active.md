...
Feature: ✅ 核准已完成 —— 下一個焦點是「第一批 story 從哪個系統開始」
Task: 🟢 **2026-08-25 第七批。ADR-0002 與 ADR-0003 已 `Accepted`,本專案第一次解除 `src/` 正式開發的自動阻擋。** 七個檔案的核准登錄全部落檔並經協調者複驗;`/story-readiness` 的依賴鏈漏洞同批補上(含循環偵測)。

**已核准**:ADR-0002(好感度數值池)、ADR-0003(存檔序列化格式)。**仍為 `Proposed`**:ADR-0001、0004、0005。

⚠️ **下一個 session 的判斷前提**:`ADR-0004 → ADR-0003 → ADR-0002` 這條鏈上,**ADR-0004 本身仍是草案**,所以任何引用 ADR-0004 的存檔 story 依舊會被擋(而且現在會被**正確地**擋,不再誤放行)。可立即開工的是**只引用 ADR-0002 的好感度資料層 story**。
