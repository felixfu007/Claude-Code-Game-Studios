...
Feature: 兩份 ADR 一起核准(存檔格式 + 好感度數值池)
Task: 🟢 **2026-08-25 第六批。兩份 ADR 的五個核准條件都已備齊,唯一剩下的動作是管理者裁決核准 —— 那是新 session 的第一件事。** 三個提交已推送:`cbe18e3`(探針)/`48402bb`(ADR-0002)/`d933e1c`(ADR-0003 + 登記表 + 參考庫)。工作區乾淨。

**ADR-0002**:五條件**全部成立**。**ADR-0003**:條件一~四成立,**條件五等同一次核准動作**(`Depends On` 指向 ADR-0002,門檻文件的預設做法是兩份一起核准)。

🔴 **核准動作要改的不只兩行**:兩份的 `Status`、`adr-revision-history.md`、**以及每次對話都會載入的 `.claude/docs/technical-preferences.md`**(它現在仍寫「五份全部停在 `Proposed`,無一 `Accepted`」與那張現況表)。⚠️ 門檻文件第 180–188 行明文:`Status:` 那一行**是程式會讀的**,旁邊的說明文字不是 —— 只加註記等於沒核准,而且比留在草案**更危險**(開發被自動放行、人卻看得到警告)。
