...
Task: 🟢 **2026-09-01 第二十六批:擋著開工的事已清到只剩引擎實測。** ①第 8 個月停損點**已同意**(機制同意,門檻數字須在第 6 個月前、看到數據前定死);②ADR-0005「單一根 Viewport 假設」**兩半皆關閉**;③🔴 **`TD-ADR` 關卡首次對 ADR-0001 與 ADR-0005 執行**(自專案成立從未跑過),兩份皆 CONCERNS,修訂已落地,**核准門檻條件四成立**;④`board_version` 改為屬性攔截(實測證據 `prototypes/adr0001-board-property-spike-2026-09-01/`)。**下一步:條件一的引擎探針(0001 四支 / 0005 六支)已派工,回來後兩份即可送核准。**
<!-- /STATUS -->
...
| # | 事項 | 狀態 |
|---|---|---|
| 1 | **ADR-0005 的「單一根 Viewport 假設」重新裁決** | ✅ **2026-09-01 第二十六批:兩半皆已關閉,管理者設的解凍前置已達成。**<br>**世界層那一半**:2026-08-27 就實測結案(`prototypes/board-render-input-spike-2026-08-27/`,判定**條件成立**,20/20 自洽 + 主 session 獨立複驗),只是**沒人把結果寫回任何文件**,五處記錄一路寫著「尚未處理」直到本批才發現。<br>**介面層那一半(VR #11b)**:由 `prototypes/ui-canvas-scale-spike-2026-09-01/` 實測 + 管理者畫面架構裁決關閉。**關鍵是它的形狀跟我們問的問題不一樣** —— 決定安不安全的不是介面基準畫布選什麼,而是**游標圖層有沒有自己獨佔一顆節點**,而 ADR-0005 本來就是這樣設計的,專屬節點下四種解析度實測全部恆等。<br>🔴 **留下一條實作義務,不是「沒事了」**:誤把游標圖層與介面圖層混成同一顆節點,實測誤差 1080p 1440px / 2K 2178px / 4K 3304px(游標系統實質失效)。**必須寫成一條會執行的自動化測試。**<br>⚠️ **ADR-0005 本體仍未動、仍為 `Proposed`。** 前置達成 ≠ 已核准 —— 它還要走 `adr-acceptance-criteria.md` 的核准門檻(含從未對它跑過的架構總監覆核)。 |
...
🔵 **有兩件事擋著全部正式開發:ADR-0001 與 ADR-0005 都仍為 `Proposed`。**
