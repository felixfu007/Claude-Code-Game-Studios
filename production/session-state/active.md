...
<!-- STATUS -->
Epic: 🔴 **專案想像重新校準** —— 管理者提供遊戲流程雛形,與現有 9,300 行文件存在多處矛盾
Feature: 七方落差審核已完成,**零項已裁決**。7 件事待管理者拍板
Task: 🟡 **2026-08-25 第九批。管理者口述遊戲流程雛形(克蘇魯美術與劇情 + 戰棋 + 卡牌、十關、三結局),經 7 位專家獨立審核。** 原話逐字存於 `design/gdd/gameplay-flow-draft.md`(62 行);審核報告 `docs/reviews/gameplay-flow-gap-analysis-2026-08-25.md`(421 行)。

🔴 **接手第一件事:讀那份審核報告,不要只讀本節摘要就動手。** 本節是索引,報告才是內容。

**一句話**:雛形與現有專案不是同一款遊戲的兩種規模,是兩款不同的遊戲。但要丟的是**排序**,不是**內容** —— 9,300 行文件幾乎沒有一份要作廢。

## 三個已失效的判斷(第八批當天做的,當天就被推翻)

1. 把存檔系統降級(`systems-index.md:31`)—— 強制閃退旗標與十關跨場次狀態使它變回必要
2. 一年計畫「不需要有的:多結局」(`one-year-plan.md:48`)—— 雛形要三個結局
3. **MVP 九個系統這份清單**(`systems-index.md:66`)—— 降級的 #2 #7 #12 全部變回必要,原在垂直切片的 #13 敘事分支也變必要。**只有 #14 活棋盤地形演變仍可不做。**

⚠️ **因此第八批 STATUS 寫的 MVP 施作順序(1→3→4→5→6→8→9→10→11)已不可用,不要照它排工作。**

## 待管理者裁決(7 項,依序;第 1 項決定其餘全部)

1. **核心幻想**:「算無遺策的軍師」vs「牽手不發瘋」
2. **低語卡是否隱藏扣分** —— 要隱藏就得撤掉支柱一與支柱三,並明文記錄本作放棄「後果完全可預判」
3. **「多變的強化效果」語意** —— 牌池多樣(不踩線)vs 打出時抽選(踩 `rng_in_combat_settlement`)
4. **三結局改用什麼判定** —— 「正向達標」已實測證明區辨力為零(`game-concept.md:113`)
5. **5 次心靈對話 vs 10 對配對** —— 數學上一半永遠不可達,撞支柱級約束(`game-concept.md:69, 71`)
6. **強制閃退是否保留** —— 主機認證判定會擋(信心「高」),且會**靜默失效**(旗標可能未落盤)
7. **兵分二路是否保留** —— 要修從未被任何人檢查過的 ADR-0001(兩個盤面會假通過)

## 不受雛形影響、現在就能動工的

🟢 **好感度資料層寫成程式** —— 在雛形之下重要性**上升**(它保留完整歷程,正是「達標判定」需要的資料),且是唯一能換到實作速度數據的工作。**這一項不必等裁決。**
🟢 **五名主角立繪可以先畫** —— 任何版本都要。其餘美術資產暫緩(多了 7~9 類未計入工時)。

## 已停的工作

🔴 依「九個系統」規劃任何新設計文件(會漏掉四個必要系統)
🔴 按「5~8 場關卡」做內容或關卡規劃(實際 12 關)
🔴 推進第 8 個月停損點裁決 —— 它量的是回合數與重傷數(純戰鬥指標),雛形重心在關係與敘事。**先確認要量的是不是同一件事。**

⚠️ **`.gd` 檔依檔案路由表歸 `godot-gdscript-specialist`** —— 環境規則要求使用者未要求時不得呼叫 Agent 工具,**開始寫程式前必須先向使用者提出此衝突,不可靜默處理。**
本批的 Agent 授權來自使用者明確指示「請各專業 Agent 審核」,**不自動延伸到下一批。**
<!-- /STATUS -->
