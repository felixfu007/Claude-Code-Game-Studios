...
登記於 `art-direction.md` 第七節:世界層放大倍率(2K 放大 5 倍窄邊 vs 4 倍寬邊)、
...
- **戰鬥數據不顯示**(2026-08-28 管理者玩後回饋,**已排入 C 批**):看不到敵我血量數字、
  看不到攻擊後的血量變化、**動手前無傷害預判**。
  🔴 **管理者的理由改變了需求範圍**:他說的是「攻擊後的血量變化」,但理由是
  「**無法順利下好一盤棋**」—— 下棋是動手**之前**的事。因此交付項不是事後回饋,
  而是**預判**:游標指向敵人時顯示「12 → 5」。原型 v5 已驗證此形式(預判標記 + preview 面板)。
  **與好感度加成強耦合**:傷害 = `max(0, ATK − DEF + Φ)`,先做預覽會在 Φ 接上後算錯,
  故與 C 批(畫線)同批實作,只做一次。
  **協調者已裁定不另外詢問的一項**:敵人血量顯示**精確數字**而非模糊血條 ——
  本作支柱是「戰鬥裡沒有任何一件事是你算不出來的」(`game-concept.md`),零 RNG 的
  全部意義就在此。藏數字與該支柱直接衝突。若日後玩起來過於透明,那是可調的裁決,
  但預設應為全部攤開。
