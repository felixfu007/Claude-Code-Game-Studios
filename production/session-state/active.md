...
Task: **2026-08-24 第五批。存檔格式那份(ADR-0003)修訂完成、兩道正式關卡通過(TD-ADR 為本專案史上首次執行)、證據已搬離至參考庫、已推送 —— 但 `Status` 仍為 `Proposed`,因為它依賴的 ADR-0002 還是草案,改了也不解鎖開發。管理者裁決:兩份一起核准。** ADR-0002 兩道關卡皆 `CONCERNS`:三項必修中**必修 1 已修(誤收進提交 `27e5ea4`,該提交訊息在講核准門檻)**,**必修 2(`ImportResult` 全文從未宣告,而它是 ADR-0003 唯一消費的兩個介面的回傳型別)與必修 3(第 657 行兩參數 `bytes_to_var`,一行)未完成**;兩支阻擋級探針中 **VR11 已跑完**(`[]` 越界**會中止**呼叫端、`.get()` 越界**不中止**回 `<null>`、負索引 -1 在非空陣列可用)、**VR9 未完成**(探針自己編譯不過,`runner.gd` 呼叫了未定義的 `_classify_default_before_nil()`)。🔴 **協調者本批三次犯同一模式:在一個正確的事實旁邊多推導一步** —— (1) 補救補在錯的層級(那條規則本來就在 `/architecture-review` 與 `/gate-check`,**真漏洞是 `/story-readiness` 不追依賴鏈,仍然開著**);(2) 寫進參考庫的宣稱被實測否證,**且有兩份副本**;(3) 引用的證據本身沒說我說的那件事。四處編輯已由參考庫擁有者撤回並重新附證,門檻條件五已由架構總監逐字改寫(初稿的「加註記後照樣核准」會拆掉唯一真的會擋人的閘門)。**接手先讀 `active.md` 檔末「第五批」與 `PENDING-AFTER-WRITE.md`。**
<!-- /STATUS -->

**最後更新**:2026-08-24(**第四批**)—— 探針 F/G/H/J 四支 + ADR-0003 修訂草案 + Step 5.5 雙軌覆核(**兩軌皆判不可寫入**)。提交 `324fb02`/`27217ec`/`aee86fc`。**詳見檔末最後一節** —— 那一節是 scratchpad 草案的 durable 替代品,草案本身新 session 讀不到。
