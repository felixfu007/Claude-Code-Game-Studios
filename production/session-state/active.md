...
Feature: 垂直切片《迷霧岔口》。🟢 **可以玩了** —— 棋盤畫得出來、滑鼠與手把都能操作、回合會推進、敵方會自己動。下一步是真人玩一場並量數字
Task: 🟢 **2026-08-27 第十八批。接線完成:`BattleScreen` + `DeviceAuthority`。145 個測試全過、0 孤兒、exit 0,主 session 獨立複驗 + 實機啟動零錯誤 + 自行補拍三張證據截圖(鍵盤確認鍵、結束回合→敵方階段整條路)。🔵 下一步:真人玩一場,量對局長度與丙的存活率。**
...
| **測試** | ✅ **145 個(原 129 + 游標主權 10 + 游標夾邊界 6),14 個 suite,0 失敗、0 錯誤、0 孤兒、exit 0。主 session 獨立複跑 + 實機啟動 12 秒零錯誤零警告** |
...
10. 🔵 最小的「這兩人現在給你 +N」提示(**不做整套好感度 UI**)。⚠️ 送入管道已存在:
    `BattleController` 建構子第三參數 `phi_provider`,已有測試涵蓋
11. 🟡 補畫面證據:攻擊結算與勝敗顯示(真人玩一場會順便驗到)
12. 🟡 補掉 `battle_loop_test.gd` 那條「閘門失效時會掛住而不是失敗」的舊測試
