...
- 🆕 **`coding-standards.md` 的 CI 指令列在本機叫不動**(見上方第一點),尚未修文件
- 🆕 **證據驗收標準需要制度化** —— 目前「顏色數」擋不住開機畫面,新增的兩道防線只寫在腳本裡,未寫進 `coding-standards.md`
- ~~匯出版無畫面證據~~ → ✅ 2026-08-31 已關閉
- ~~`battle_loop_test.gd` 會掛住~~ → ✅ 2026-08-31 已關閉
- ~~`tools/` 工具不進版控~~ → ✅ 2026-08-31 已修(四條 gitignore 規則錨定到根)

### 下一步(接手順序,2026-08-31 第二十二批更新)

1. 🔵 **管理者玩一場**(`PLAY.bat`),量對局長度、丙的存活率、格線有多痛
2. 🔵 **把 Release 網址發給外部測試者**,搭配回饋單
3. 🟡 依量到的數字談難度 —— **在那之前不要動任何平衡數值**
4. 🟡 把「證據自證的兩道防線」寫進 `coding-standards.md`,別只活在一支腳本裡
5. 🟡 修 `coding-standards.md` 的 CI 指令列(本機需完整路徑)
