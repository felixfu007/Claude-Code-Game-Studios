...
**規則 1 有三項明文例外(不拆)**:`affinity-position-chain.md`、`tactical-combat-system.md`
(兩者 2026-08-31 裁決)與 `skill-card-system.md`(2026-09-08 裁決,第一輪 `/design-review`
17 個叢集全部落在 native design,管理者裁決保留丙類;`creative-director` 事前估算
保留丙類約 420 行,實測方向一致)。
**是這三份檔案本身,不是某個行數。** 理由與登記見計畫檔第六節②。

🔴 **三份例外之後,實務判準已是「個案裁決」而非 400 行 —— 這是管理者在知情下的選擇。**
裁決當時協調者明文告知代價為「例外第三次之後,上限實質上就不存在了」,並提供了不破上限的
替代方案(拆分),管理者仍選擇登記例外。**故凡以「規則 1 是硬性上限」為由要求拆分文件前,
請先讀計畫檔第六節②那段。** 要讓它恢復為真正的上限,需要一次明文重申加一個會執行的檢查,
不是再加一份例外。
🔴 **是例外不是放寬,其餘文件仍為 400 行。**

⚠️ **例外登記的對象是「這三份檔案」本身,不是某個行數。** 2026-09-01 稽核:計畫檔的例外表把
`tactical-combat-system.md` 寫為 1038 行,而它**在登記豁免的同一天、後續兩次提交就長到 1077**
—— 沒有任何人發現。這正是下一句警告的事,只是它自己也沒躲過:

> 🔴 **本段兩處於 2026-09-14 更正,兩處都是本檔自己寫錯:**
> ①原寫「原文**在三處**寫死 1038 行」—— **實測是 1 處。**
> `git show $(git rev-list -1 --before=2026-09-01 HEAD -- production/milestones/one-year-plan.md):production/milestones/one-year-plan.md | grep -c 1038` → `1`
> ②原寫「例外登記的是『這**兩份**檔案』」,而同節上方已寫三份 —— **本檔前後矛盾**,現統一為三份。
> ⚠️ 計畫檔 §6② 的對應句**至今仍寫「兩份」**(2026-09-08 增列第三份時未同步)。
> **錯在來源側,已另行派工 `producer`;本檔以三份為準,不等來源修好。**
> 📌 順帶:`wc -l` 實測 2026-09-14 現值為 **1087 / 492 / 536**(登記快照 1038 / 477 / 468),
> **三份全部已漂移** —— 包括計畫檔宣稱「未漂移」的那一份。快照就是會漂,所以:
...
## 美術方向與像素風專案設定、畫面架構與 `project.godot` 引擎設定

**權威全文在 `design/art/art-direction.md`。** 2026-08-25 管理者裁決:像素風、開發者本人繪製、
**480×270 套裝**(立繪 128×128、棋盤單格 32×32、棋子 32×40、64 色、1px 描邊、
對話正文用一般繁體中文字型而非像素字型)。

🔴 **本節有兩類內容,權威來源不同、內容變更由誰發動也不同(2026-09-14 技術總監接手時標示)。**
本節標題原本只寫「美術方向」,但 2026-09-14 逐項比對實測:本節 33 項陳述中有 **13 項的
權威來源是本檔自己**。**查 `project.godot` 設定的人,不該因為標題寫著「美術」而跳過這裡。**

| 段落 | 權威來源 | 內容變更由誰發動 |
|---|---|---|
| 上方 480×270 套裝硬參數 | `design/art/art-direction.md` | `art-director` |
| 「🔴 畫面架構裁決」節 | `design/art/screen-architecture.md` | `art-director` |
| **其餘各段** —— `project.godot` 設定表、(A)/(B)/(C) 三級證據紀律、三項會反噬的實測事實、單一 Viewport 兩條件、`GameRoot.tscn` 缺陷 | **本檔自己** | `technical-director` |

⚠️ **第三類不是摘要,它沒有上游可以比對。** `art-direction.md` 第六節明文把細節**反向指回本檔**
(逐字:「細節與來源等級見 `.claude/docs/technical-preferences.md`」)。
**不要去美術文件找它們的「正本」,沒有。**
...
🔴 **同一失效模式已第三次發作(2026-09-22,Story U-013),而第三次穿過了上面這條判準。**
(2026-09-23 回寫交叉連結。)完整紀錄在 `.claude/docs/coding-standards.md` 的
「已知陷阱(Check 4,A/B/C 三類共通):分析/證據腳本繞過真實渲染管線,以及重複計數的方向性偏誤(2026-09-22)」節
—— **該檔屬 `lead-programmer` 網域,本節只連過去、不複述。** 三次的形狀:

| # | 日期 | 量測腳本做錯的事 | 是什麼發現的 |
|---|---|---|---|
| 1 | 2026-08-31 | 用 awk 自己實作一份地形通行規則(假定 `#` 不可通行) | 人 |
| 2 | 2026-09-22(U-013 證據驅動器第 1~2 版) | 自己算世界層縮放:寫死 `SCALE_FACTOR = 4`、位置留在節點預設 `(0,0)`,沒呼叫 `WorldLayout.compute_scale()` | 人看截圖問「這有置中嗎」 |
| 3 | 2026-09-22(同一支驅動器第 1~3 版) | **沒有重新實作任何規則** —— 它略過了世界層的真實宿主(`SubViewportContainer` + `SubViewport` + `world_viewport_scaler.gd`),把 `BoardView` 直接掛在根 `Window` 下手動套 transform | 第二位專家改查渲染管線 |

**三次都不是規則攔下來的。**

🔴 **第 3 次是本節判準接不住的形狀,而原因不是作者沒照做 —— 是他照做了、寫下自述、而自述每個字都是真的。**
該驅動器第 1~3 版的檔頭逐字
(`git show 6e7148f:prototypes/u013-highlight-evidence-2026-09-22/evidence_driver.gd`):
...
## calls its real public methods ... exactly as battle_screen.gd would.
```

同一次量測報 **194 / 229440** 格違規,走真實管線複驗是 **0 / 229440**。
**讓數字錯掉的那個事實,就寫在宣告它正確的同一句話裡**(`as a child of the root Window`)——
本節的判準沒有任何欄位讓那半句話產生意義。

**缺口的一句話形式**:本節判準管的是**哪些程式碼跑了**,不管**它跑在什麼宿主/場景組合裡**。
`BoardView`、`WorldLayout`、那些方法呼叫全都是真的;假的是它被掛在哪裡 ——
而 production 真正會跑的 `src/ui/battle/world_viewport_scaler.gd` 一次都沒跑到。
下方第 1 點要你「呼叫專案自己的類別」,**但它沒有區分「呼叫了我想測的那幾個類別」與
「跑完了 production 會跑的那一整條鏈」** —— 第 2 點的補救(列出重新實作的每一條規則)
也接不住,因為這次的錯是**少跑了一層**,不是多寫了一份;沒有人列得出自己不知道存在的那一層。

📌 **與下方「兩項刻意未定義的邊界」的關係 —— 是同一個形狀的另一半,不是重複**:
那裡第一項是「真的類別 + 假的輸入資料」,本項是「真的類別 + 假的宿主」。
🔴 **但那兩項當初「暫不立法」的逐字理由是「兩者都還沒真的害過人」,本項已經害過**:
那個 194 引發跨網域調查,`art-director` 依專案明文的像素對齊規範**正確地**判定「該改遊戲」,
差一步就會去改一段本來正確的 `src/ui/battle/board_view.gd`。
**故本項僅登記為已知缺口,本節不自行立法** —— 要不要把「宿主/場景組合」納入判準是一次
**新的管理者裁決,尚未進行**。在裁決之前:**本節現行文字不涵蓋這個形狀,不要當成它涵蓋。**

> ✅ **2026-09-23 更新:上一段的「尚未進行」已經發生了,結論句已被取代。**
> 管理者已裁決,條文在下方「**第 5 點**」。**本節判準現在涵蓋宿主/場景組合這個形狀。**
> 上一段原文保留作為決策紀錄(它記載了立法之前的狀態與理由),**但不要再照它的結論句行動。**
> ⚠️ 本次裁決**只**涵蓋這一項;它提到的「兩項刻意未定義的邊界」不受影響,仍為暫不立法。
...
##### 🔴 第 5 點:量測腳本必須 `load()` production 真正啟動的那份場景檔(2026-09-23 管理者裁決)

**裁決逐字錨點**(`active.md` 每批重寫,故錨在選項原文而非標題):
`grep -n "要求物證:直接載入真實畫面檔" production/session-state/active.md`;
若該檔已輪替,改查當批提交訊息 `git log --all --grep="要求物證"`。
管理者選的是**「要求物證:直接載入真實畫面檔」** —— 「維持現狀只留登記」與「只加一條紀律條文」
兩個選項**都在選單上、都被跳過了**。
📌 **這是知情下的選擇,寫在這裡是為了日後不被描述成「因為自動檢查有效」**:協調者當場向管理者
指出本專案至今所有畫面缺陷**都是人開圖看到的、自動檢查一次也沒抓到過**,管理者知道這一點仍選本案。
**本點買的不是偵測力,是讓自述之外多一份第三方可自行核對的物證。**

**條文** —— 凡量測結果要標 **(A) 級**並進入設計文件或架構決策:量測腳本**必須** `load()`
production 實際會啟動的那份 `.tscn`,`instantiate()` 後掛進 `get_tree().root`,
**不得自行組一條「型別與腳本都對」的替身節點鏈**。
上方第 1 點管的是**哪些程式碼跑了**,本點管的是**它跑在哪一條宿主鏈上**。

**適用範圍分兩段(引擎事實,不是本點的選擇)**:

| 量測種類 | 執行環境 | 本點要求 |
|---|---|---|
| **像素**(整數格、支配色、取樣那類) | **只能開窗** —— headless 下兩條取像路徑皆 `get_image()` 為 null | 必須 `load()` 真實場景檔 |
| **結構/狀態**(場景樹形狀、屬性值、旗標) | headless 亦可 | **同樣**必須 `load()` 真實場景檔,不得自組替身 |

headless 取不到像素是 dummy rendering driver 的引擎限制,**與載入哪個場景無關、也不是本點新增的**。
(A) 級來源:`prototypes/godot-specialist-scene-load-feasibility-2026-09-23/probe_load_real_scene.gd`,
原始輸出當場查:`grep -n "get_image() is null" prototypes/godot-specialist-scene-load-feasibility-2026-09-23/run_output_headless.txt`

🔴 **物證怎麼算數 —— 這是本點的核心,因為「自述」已經被合法穿過一次。**
腳本檔頭寫「我載入了真實場景」**不構成物證**(2026-09-22 那支的自述每個字都是真的)。
物證是**原始 log 裡留著真實宿主鏈自己算出來的量測值** —— 例如
`WorldViewportContainer` 的 `position`/`size`/`stretch_shrink` 與 `WorldViewport.size`:
自組替身時這些值由腳本自己決定,載入真實場景時由 `world_viewport_scaler.gd` 算出。
**留 log,不要只留結論;沒有 log 的「(A) 級」比照第 4 點降為 (C)。**

**兩個必踩的實作坑(不寫下來,每個照條文做的人都會各撞一次)**:

1. **在 `_ready()` 裡直接 `add_child()` 會被引擎當場拒絕**(`Parent node is busy setting up children`),
   headless 與開窗兩次獨立執行**逐字重現**。改用
   `get_tree().root.call_deferred("add_child", instance)` 並等 2 個 `process_frame`。
   逐字訊息:`grep -n "busy setting up" prototypes/godot-specialist-scene-load-feasibility-2026-09-23/run_output_*.txt`
2. **資料注入只有一個時機窗:`instantiate()` 之後、`add_child()` 之前**(`_ready()` 只讀一次,
   而 `_ready()` 要進樹才跑)。與第 1 點合起來的唯一正確順序是
   **`instantiate()` → 設 override → `call_deferred("add_child")` → 等幀**。

**「需要合成場景」不再是跳過真實場景檔的理由。** 同日管理者裁決把 `battle_screen.gd` 的資料路徑
全部改為可注入 —— 要合成佈局就**寫一份合成資料檔餵進去**,不是去組替身節點鏈。
現有注入點當場查:`grep -n "_path_override" src/ui/battle/battle_screen.gd`

✅ **2026-09-24 管理者裁決:本段前半的「停下來問」閘門已移除,理由消失了。**

**原文寫的是**:「這條路還沒有真的走通過一次 …… **在它走通之前**,遇到需要整組合成資料的量測:
**停下來問**,不要自行退回組替身節點鏈。」——那個閘門的成立條件是**「尚未走通」**。

**2026-09-23 第六十二批走通了**:兩份合成夾具原本彼此對不上(ENEMY 起點 `(12,5)` 落在
`5×3` 地形夾具描述的範圍外),已修為 `(4,2)`,並新增一條**同時注入 terrain + roster 兩個路徑**
的測試。**這是本專案第一次證明「同時餵一整組合成場景」可行。** 當場查:
`cat assets/data/levels/test_fixture_terrain.txt; grep "^2," assets/data/units/test_fixture_roster.txt`

🔴 **但後半那半句仍然有效,而且位階不變:「不要自行退回組替身節點鏈」。**
它不是靠「這條路沒走通」撐著的 —— 它是上方**第 5 點**明文禁令的同一件事(真的類別 + 假的宿主),
那條禁令有自己的實測依據(2026-09-22 那支驅動器報 194 格違規、走真實管線複驗是 0)。
**閘門拿掉的是「先問」,不是「可以組替身」。**

⚠️ **走通一次不等於每次都會順。** 需要整組合成資料時,成本仍是「寫夾具 + 注入 + 驗夾具彼此相容」
—— 上面那次的坑就是**兩份夾具互不相容**,而且它不會報錯,只會量出一個不存在的棋盤的答案。
**寫第二組合成夾具時,先驗兩份對得上再開始量。**

⚠️ **不要把本點讀成「載入真實場景檔比較省事」。** 實測 `BattleScreen._ready()` 幾乎自我初始化、
驅動成本近乎零(見上述探針 README 的 Q3)—— **但那只在真實資料檔剛好能表達你要驗的場景時成立**。
需要合成資料時成本回到「寫夾具 + 注入 + 驗夾具彼此相容」,**本點不會替你省下這一段。**

🔴 **本點只涵蓋「真的類別 + 假的宿主」這一個形狀。** 下一節那兩項(「真的類別 + 假的輸入資料」、
「混合情況沒有等級可標」)**未被本次裁決涵蓋**,仍為暫不立法、碰到時停下來問。

⚠️ **與本節其餘各點一樣,沒有任何 skill、lint 或閘門會檢查本點** —— 遵守與否目前不可觀測。
...
⚠️ **跨文件衝突 —— 兩半皆已實測結案**
(世界層 2026-08-27、介面層 2026-09-01;2026-09-01 補寫回本檔)
...
1. **世界層倍率/位置仍要自己寫程式管**,整數對齊自己保證 —— 這項沒有自動解決。
2. **字級規則已於 2026-09-04 管理者裁決並實作完畢,不再是待辦。**
   規則全文、推導理由與逐解析度實測表在 **`design/art/screen-architecture.md`
   的「一之二、HUD 字級規則」節**。🔴 **本檔刻意不複述那條公式** —— 理由見本節開頭的
   權威來源表:複述會開出一個新的單向同步面,來源日後改了,這份每次對話開場載入的
   文件就會安靜地說著舊的。**要公式就去讀那一節,或直接呼叫下面那支函式。**

🔴 **隨該裁決生效的硬性實作義務**:HUD 字級所需的倍率 `N` **必須呼叫
`src/ui/battle/world_layout.gd` 的 `compute_scale()` 取得,不得自行計算,不得複製公式。**
現行 `src/ui/battle/hud_layout.gd` 即是如此。本專案已有「同一個公式兩份實作、只是今天
答案一致」的登記案例,而黑箱比對輸出**永遠無法區分「共用同一份」與「兩份碰巧一致」**。

⚠️ **已知代價是裁決接受的,不是缺陷**:螢幕尺寸不是 480×270 整數倍時,字會「掉檔」。
最明顯是 **1366×768(常見筆電)只拿到 `N=2`**,與 960×540 最小視窗同字級。
🔴 **不要當成 bug 去修** —— 要改是一次**新的裁決**(等於與棋盤倍率脫鉤,會失去
「單一出處」這個最大好處)。

🔴 **仍未決、且會擋路的一項**:**對話正文字型只定了類別**(一般繁體中文字型、非像素字型),
**具體字型尚未選定**。管理者 2026-09-04 裁決本階段不選(當時沒有對話畫面可驗證)。
**排對話畫面之前必須先選定,並為向量字型另訂一套字級規則** —— 上述 HUD 字級規則是
**點陣字專用**,其整數倍限制對向量字型不適用。
(來源:`screen-architecture.md`「⚠️ 尚未涵蓋:對話正文字型」節。)
...
✅ **2026-09-01:ADR-0001、ADR-0005 兩份已 `Accepted`**(管理者裁決)。**五份中已有四份核准,
只剩 ADR-0004(存檔原子寫入)仍為 `Proposed`** —— 而它依一年計畫第七節**明確不在 12 個月範圍內**
(「不在最短路徑上,維持 `Proposed` 即可,不要再投入」)。

**兩份的核准依據**:五個條件逐條成立,全文寫在各自檔案的 `## Status` 節。關鍵在於
**`TD-ADR` 架構總監覆核於 2026-09-01 首次對這兩份執行**(該關卡自專案成立從未對它們跑過,
成因見 `docs/consistency-failures.md`),兩份皆判 CONCERNS,修訂後各經一次**窄範圍複驗**
—— ADR-0001 那輪**判不通過**、指出三處事實錯誤,更正後才放行。
引擎探針:ADR-0001 四支全過、無一推翻;ADR-0005 六項關五項,#5(Steam 疊加層)本機測不了、
已誠實登記為不阻擋。

🔴 **兩條隨核准生效的實作硬性義務**:①游標圖層必須獨佔一顆 `CanvasLayer`,不得與介面圖層共用;
②機制四之二必須自行過濾 `InputEventKey.echo`(引擎 `event_is_action()` **不過濾**,已實測)。

⚠️ **核准不等於全部能開工**:`tactical-combat-system.md` 另受其自身 **OQ-2(我方基準數值表無人擁有)**
阻擋,ADR-0001 不是它唯一的鎖。
...
| ADR | 主題 | 最近動作 | 現在卡在什麼 |
|---|---|---|---|
| 0001 | 戰棋查詢介面原子性契約 | ✅ **2026-09-01 `Accepted`** | 可開始寫正式程式碼。⚠️ **但戰棋系統仍不能開工** —— 它自己還卡 OQ-2(我方基準數值表無人擁有)。未涵蓋項見該檔 `## Status`
...
| 0005 | 單一游標/高亮狀態系統:裝置權威輸入架構 | ✅ **2026-09-01 `Accepted`** | 可開始寫正式程式碼 —— **#3 游標/高亮是四個已核准系統中第一個真正解鎖的**。🔴 兩條硬性義務隨核准生效(游標圖層獨佔節點、自行過濾鍵盤 echo)
