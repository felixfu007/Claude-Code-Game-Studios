...
**規則 1 有兩項明文例外(2026-08-31 裁決,不拆)**:`affinity-position-chain.md` 477 行、
`tactical-combat-system.md` 1038 行。理由與登記見計畫檔第六節②。
🔴 **是例外不是放寬,其餘文件仍為 400 行。** 另注意:**行數上限沒有自動檢查** ——
1038 行那份超限 2.5 倍,是超限生效六天後被順手量到才發現的。
...
| ADR | 主題 | 最近動作 | 現在卡在什麼 |
|---|---|---|---|
| 0001 | 戰棋查詢介面原子性契約 | 2026-08-18 建立,**從未修訂**;引擎專家已於 `#4` 第四輪查核(2026-08-31 更正,見下) | ⚠️ 卡在核准門檻**條件一與條件四**:①它自列的 **6 項待驗證全部未做**(需碰真引擎);②**架構總監覆核從未跑過**(這一項對五份 ADR 皆然,非本份獨有) |
