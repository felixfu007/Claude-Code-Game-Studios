...
**Permanent Lockout Event Check (mandatory when a fix adds or rewrites a rule) — added 2026-08-12, round 10 of `/design-review save-system.md`, disambiguated round 11**: this skill is, empirically, the dominant channel through which this project's GDDs receive substantive rule changes — more so than `/design-system`, `/quick-design`, or `/reverse-document`, which is why those three already carry an equivalent check ("5a-ter" / the quick-design redirect bullet / reverse-document's Phase 5b) but this skill, the busiest of the four, did not. If any blocking-item fix you are about to write introduces or rewrites a rule that makes it permanently impossible to trigger a specific relationship-affinity source for a specific character pairing, treat it exactly as `/design-system`'s 5a-ter does: add a row to `design/gdd/affinity-data-pool.md` §3g's registry, then check whether **this review round's own target file is `game-concept.md`**:
- **If yes** (this round is reviewing `game-concept.md` itself): confirm D-1's generalized rule covers the new instance, and if it doesn't, fix D-1 in this same round — this round *is* the `/design-review` pass D-1's maintenance protocol calls for.
- **If no** (this round is reviewing any other GDD, the common case): confirm D-1's generalized rule covers the new instance **without modification**. If it does, record that confirmation. If it does not, **do not edit `game-concept.md` from this round** — round 11 found this branch's original wording ("or note that this review round itself is the escalation") ambiguous enough to be misread as license to patch D-1 inline from a review round targeting a different file, which defeats the whole point of routing D-1 changes through their own reviewed pass. Instead, stop and flag that a separate `/design-review design/gdd/game-concept.md` round is required, and note that flag in this round's revision-list entry.

Either way, record the confirmation result in this round's revision-list entry.
