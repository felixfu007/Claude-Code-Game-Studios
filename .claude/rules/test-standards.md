...
🔴 **不得把本節當成「檔案 I/O 已獲准」。** 現在確定的只有三件事:
①這條規則有十處違反;②其中八處有另一條專案規則替它背書;③**十處全部沒有任何自動檢查,
違反不留痕跡** —— 這正是本專案反覆登記的那個形狀(規則存在、無人執行、不執行不可觀測)。

**待管理者裁決的是「規則本身該怎麼改」,不是「要不要追認這十支」。** 三個方向:
把甲類寫成明文例外、把乙類另立為「治理測試」類別(它斷言的不是遊戲行為而是原始碼紀律)、
或維持禁令並要求十支全部改寫。**在裁決之前,新增測試若要碰檔案系統,照現行規則
它是違規的 —— 請在該測試檔檔頭寫明理由並回報,不要沉默地照做。**
