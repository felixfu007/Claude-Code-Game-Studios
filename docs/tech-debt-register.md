...
- **2026-09-08**(Story 005 整幀緩衝 + `_process` 裁決 + 六行為者定序):
  🔴 **AC-32 與 AC-20 的測試沒有被證明會紅 —— 敏感度證明未做。**
  那幾條測試現在是綠的,但**沒有人故意把程式改壞、確認它們會轉紅**,
  因此無法排除「它們永遠綠、其實什麼都沒驗到」這個可能。
  **這是刻意登記為債,不是遺漏**:2026-09-07 協調者判定「潦草的敏感度證明比沒有更糟 ——
  它會讓人以為那幾條測試已經證明過會紅」;2026-09-08 管理者裁決確認登記為債、不在該輪執行。
  ⚠️ **成本要先知道**:本專案實測,一條測試失敗會**中止同 suite 剩餘測試**,故**一次注入只能
  證明第一條抓到它的那條測試**。要證明後面幾條,必須把前面的 `test_` 函式暫時改名移出收集
  範圍、逐條跑、再改回 —— **每證明一條約等於一次完整測試執行**。排程時請以此為基數。
...
- **2026-09-03**(TR-cursor-016 移除作業的範圍外發現,架構端專家提出、協調者查證屬實):
  **`docs/registry/architecture.yaml` 的 `cursor_unregistered_surface_hover_visibility` 條目
  仍是 R6-8 之前的描述** —— 逐字寫著 hover 判定發生在
  「the priority=50 global visual layer **node**」(單數、同一顆節點),
  而 ADR-0005 的 R6-8 修訂早已把自繪游標與 hover 判定器拆成**各自獨立的節點**。
  ⚠️ **與 2026-09-03 的待機指示裁決無關**,是先前就存在的漂移,只是在這次逐條 grep 核對時才被看見。
  🔴 **這是本專案的典型形狀**:同一事實的多份複本,ADR 那份改了、登記表那份沒改。
  **修它需要 R6-8 的完整脈絡**(拆成幾顆、各自職責),不在當次授權範圍內,故登記而不動。
  追蹤來源:`docs/registry/architecture.yaml` 第 1352 行起、ADR-0005 機制十三之二 R6-8 段

- **2026-09-03**(Story 012 開工前查核,協調者):
  **機制十四的「兩項條件」是一條寫在文件裡、沒有任何程式會檢查的紀律,而 Story 012 現在做會假性通過。**
  實測 `grep -rn '\.register(' --include='*.gd' src/ tests/` —— **正式程式碼零命中**,
  註冊表是空的。Story 012 的兩條驗收標準都以「全部已註冊表面」為定義域,**空集合上全稱斷言恆真**。
  🔴 **同一失效形狀的第三次**(前兩次:Story 003 的 AC-51、Story 007 的 AC-32)。
  **需裁決:兩項條件在哪裡強制執行。** 協調者建議內建進 `CursorSurfaceRegistry.register()`
  (把靠紀律遵守換成結構上不可能違反),但那會改變 `register()` 的契約、可能新增 `RegisterResult`
  列舉值,屬 ADR-0005 管轄,**不由工作單決定**。三個落點與各自能不能寫出「會失敗的測試」
  已列在工作單文末。工作單狀態已改為 **Blocked**。
  追蹤來源:`production/epics/cursor-highlight-state/story-012-disable-native-focus.md` 文末

## Resolved

- **2026-09-08 開,✅ 2026-09-08 同日關閉**(Story 005,**此項從未是債,是一項待裁決**):
  **`TR-cursor-012`「雙輸入簽章」與 GDD Core Rules #2「兩項資料」曾與實作不符。**
  `set_target()` 的第二參數 `from_ui_action` 依 2026-09-03 授權於 2026-09-07 刪除
  (刪除前已以測試證明傳 true/false 行為完全相同),**但當日沒有人同步上游文件** ——
  一個獲授權的刪除,留下一條要求它存在的 Approved GDD 規則與一條 `active` 需求。
  ✅ **管理者 2026-09-08 裁決結案**:GDD Core Rules #4 由「條件式」改寫為**不變式**
  ——**系統主動改標永不變更操作權,無條件**;Core Rules #2 改為一項資料;
  `TR-cursor-012` 需求原文**重新措辭**為「單輸入簽章」(**非取消需求本身**,
  對照先例 `TR-cursor-016` 才是取消需求)。
  **裁決依據三點**:①該條自述動機是「幾何來源缺口」而非裝置權威需求,且該動機已由 GDD
  同條第九輪註記宣告前提不成立;②它指向的 Core Rules #4 只定義了一半,「若為 `ui_*` 觸發
  則權威轉移給誰」全文從未定義,是一個沒有去處的鉤子;③實作已證明兩種傳值行為相同。
  ⚠️ **2026-08-04 第五輪原始審查記錄不存在**(審查記錄檔第六輪才建立、其開頭明文承認
  第 1–5 輪細節從未被持久化;該 GDD 亦於第五輪後才進版控,git 無 diff)——
  **裁決是在明知此空白的前提下做的,不是認定當初沒想過。**
  🔴 **下游若未來需要「主動改標連動裝置權威」屬全新設計(須先定義轉移給誰),
  不得以 GDD 或 ADR 原文為依據直接實作。**
  **同批對齊 9 個檔案**(含 `docs/registry/architecture.yaml` 凍結簽章登記與
  `docs/architecture/traceability-index.md` 兩列 —— 後者當時正把一個已知不成立的需求
  顯示為「✅ 已涵蓋」)。全文與傳播範圍見 `docs/consistency-failures.md` 2026-09-08 條目。
...
- **2026-09-04**(ADR-0005 機制十一裁決的架構覆核發現,`godot-specialist`):
  **`design/gdd/cursor-highlight-state.md` Core Rules #7 情境(乙)有半句話已經過期,
  而同一句話的另外半句是本專案一條裁決的決定性依據。**
  該句原文同時寫著兩件事:①乙分支「與『新開局進入戰鬥』**走完全相同**」;
  ②「**不需要新介面**」。
  🔴 **①仍然成立且是硬證據**(2026-09-04 管理者裁決「新開局與讀檔走同一入口」就是靠它);
  **②已被 ADR-0005 的 R5-1 修訂推翻** —— 該修訂**真的新增了一個公開介面**
  (`handoff_after_mount()`),ADR 自己也誠實記帳「介面由 5 增為 7」。
  ⚠️ **該字面矛盾自 2026-08-19 R5-1 起存在,至今沒有任何一輪 `/design-review`
  或架構審查點名處理過。**
  **判定:ADVISORY,不阻擋任何人。** 覆核者的推理是:GDD 在「規則層」講「這不是新行為」,
  不是對 Godot 方法簽章數量的承諾,兩者不在同一個抽象層級。
  📌 **但這是覆核者的推論,不是查到的裁決** —— 那句話目前仍是字面不一致的文字。
  🔴 **不建議為此去改那份已 Approved 的 GDD**(要走修訂流程,成本遠高於問題),
  但**下一次該檔因其他理由開修訂時,應順手處理這半句。**
  ⚠️ **改的時候只能改②,不能碰①** —— ①是那條裁決的依據,動到它會讓裁決失去支撐。
  追蹤來源:`docs/architecture/adr-0005-cursor-device-authority-input-architecture.md`
  機制十一「🔴 `新開局` 走哪一個入口」段
