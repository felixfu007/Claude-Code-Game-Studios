...
## 2026-09-03 下午追記:上述開放問題已查證關閉,判定「歸 Story 011」

主 session 於本日下午派工 Story 007 前依既有要求先查證此項(交接文件明文「派工前須先查證,
不要類推」)。**答案是肯定的:`reapply_native_cursor_visibility()` 的撰寫歸屬為 Story 011,
故 Story 008 的前置須加 011。**

**五條證據,強度與其餘五項「少列」判定同級**:

| # | 證據 | 位置 |
|---|---|---|
| 1 | 該方法列於 `cursor_state.gd` 的 Key Interfaces 區塊(L.1373–1433)內,註記 `# Core Rules #5` | ADR **L.1432** |
| 2 | 其主體轉呼叫 `_reapply_native_cursor_visibility_with_unregistered_surface_exception()`,該私有方法屬機制十三之二 | ADR **L.1124 / L.1126** |
| 3 | Story 011 明文宣告擁有「機制十三、機制十三之二」 | `story-011` 第 18 行 |
| 4 | Story 008 只宣告「機制九」,不含十三 | `story-008` 第 18 行 |
| 5 | 呼叫點位於機制九章節內 | ADR **L.735** |

**已更正兩處欄位**:`story-008` 的 `Depends on` 補 011;`story-011` 的 `Unlocks` 補 008。

📌 **開工順序不受影響** —— 011 本來就在第五波、008 在第六波。這是相依欄的更正,不是排程變更。
六波順序維持:`001` → `003,004` → `002,006` → `007,010,012,014` → `005,011,009` → `008,013`

⚠️ **一併記下這次查證揭露的一個結構性事實**:`CursorState` 的公開介面是**多張工作單共同組成的**
—— 光是 Key Interfaces 的查詢方法區塊(L.1427–1432)六個方法就橫跨機制十(Story 007)、
機制九(Story 008)、機制十三(Story 011)三張。**「哪張工作單寫哪個方法」在 ADR 裡沒有一張總表**,
只能逐方法回推它屬於哪個機制、再查哪張工作單宣告了該機制。
本次 Story 007 派工時即因此產生範圍歧義(派工單寫「讀取介面」,而該詞在 ADR 對應六個方法、
其中兩個不屬本張),已於派工後即時補正。**後續每一張碰 `CursorState` 的工作單都會再遇到同一個歧義。**
