...
| **Knowledge Risk** | HIGH——本 ADR 的正確性直接倚賴 `var_to_bytes()`/`bytes_to_var()`、`HashingContext` 的精確簽章與邊界行為;`breaking-changes.md` 4.3→4.4 表格已確認 `FileAccess.store_*` 系列回傳型別改變(見下方 Post-Cutoff APIs Used),但 `var_to_bytes`/`bytes_to_var`/`HashingContext` 本身未見任何版本記錄,無專屬 core/scripting 模組參考文件可查,亦無 Godot 執行環境可實機驗證 |
...
- `docs/architecture/adr-0004-save-system-atomic-write-and-migration-execution-model.md`(2026-08-18 新增)——消費本 ADR 定義的位元組緩衝區與雜湊機制,`TR-save-*` 系列至此三份 ADR 全數覆蓋。
