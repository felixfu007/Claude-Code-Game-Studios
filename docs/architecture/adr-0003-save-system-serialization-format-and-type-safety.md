...
| **Knowledge Risk** | HIGH——本 ADR 的正確性直接倚賴 `var_to_bytes()`/`bytes_to_var()`、`HashingContext` 的精確簽章與邊界行為;`breaking-changes.md` 4.3→4.4 表格已確認 `FileAccess.store_*` 系列回傳型別改變(見下方 Post-Cutoff APIs Used),但 `var_to_bytes`/`bytes_to_var`/`HashingContext` 本身未見任何版本記錄,亦無專屬 core/scripting 模組參考文件可查(2026-08-21 覆核:`docs/engine-reference/godot/modules/` 現有 animation/audio/input/navigation/networking/physics/rendering/ui 八份,**確實沒有 core 或 scripting**;對 `breaking-changes.md`/`deprecated-apis.md` 搜這三個 API 名亦零命中——兩個理由至今仍為事實)。⚠️ **2026-08-21 修正**:本欄原本以「**亦無 Godot 執行環境可實機驗證**」作為 HIGH 的**第三個理由**——**該前提已被推翻**:Godot 4.7.1 可在本機 headless 完整執行(`which godot` 找不到只代表不在 `PATH` 上),2026-08-20~21 已跑兩批 spike 與五支探針,未過濾 log 歸檔於 `prototypes/`。**HIGH 評級維持不變**,但理由收為上述兩項:**文件缺口是真的,不可實測不是**。連帶後果:下方 Verification Required 五項**全部可用低成本探針關閉**(探針 D 為 4 個 `.gd` 檔 + 一個 6 行場景),它們仍開的原因從此只能是「尚未撰寫探針」,不能再是「無法查證」——**這五項應列入下一批探針**,其中第 2 項(`bytes_to_var(bytes, false)` 對本應解碼出 `Object` 的輸入是否安全失敗)是本 ADR 型別安全論證的地基,優先級最高 |
...
| **Verification Required** | (1) `var_to_bytes(value: Variant) -> PackedByteArray` 與 `bytes_to_var(bytes: PackedByteArray, allow_objects: bool = false) -> Variant` 的確切簽章在 4.7.1 是否與訓練資料一致(`godot-specialist` 高信心度但未實機驗證);(2) `bytes_to_var(bytes, false)` 對「位元組本應解碼出一個 Object/Resource」的輸入,實際行為是否確為「整個容器解碼呼叫原子性失敗、回傳 `null`,並伴隨 console/log 錯誤訊息」(`godot-specialist` 依訓練資料對 `decode_variant()` 遞迴解碼邏輯的推測,中高信心度,**非本專案實機驗證**)——這是本 ADR 型別安全論證的地基,必須實機確認,不可假設;(3) `HashingContext` 的確切類別/enum 成員名稱(`HashingContext.HASH_SHA256`,或完整路徑 `HashingContext.HashType.HASH_SHA256`)與 `start()`/`update()`/`finish()` 方法簽章(`godot-specialist` 高信心度,古老穩定 API);(3a) `PackedByteArray`/`String` 是否提供 `sha256_buffer()` 等一次性便利方法可取代 `HashingContext` 三段式呼叫(`godot-specialist` 低信心度觀察,若存在為簡化機會,不影響現行設計正確性);(4) `Dictionary` 巢狀 `PackedByteArray` 值透過 `var_to_bytes()`/`bytes_to_var()` 往返是否保真、且外層解碼時是否確實不會遞迴解讀 `PackedByteArray` 內容本身(`godot-specialist` 高信心度——`PackedByteArray` 在 Variant 編碼中為一級型別,不透明位元組blob,理論上不會被遞迴解讀,**未實機驗證**);(5) `var_to_bytes()`/`bytes_to_var()` 對超大 `PackedByteArray`(數百小時遊玩累積的 Delta Log)是否有效能懸崖或大小上限(理論上限為 int32 長度欄位的 ~2GB 量級,遠超本專案估計規模,風險低但未驗證) |
...
**每個區塊的 payload 各自獨立序列化為一份 `PackedByteArray`**(而非把所有區塊的 `Dictionary` 直接攤平進外層結構),再把這些緩衝區當作值放進外層 `blocks` 字典——這個分層是本 ADR 對 `TR-save-012`(manifest-only vs 完整讀取雙路徑)的具體實現機制:外層結構透過**一次** `bytes_to_var(buffer, false)` 呼叫解碼,得到 `ruleset_version`/`block_manifest`/`top_level_hash` 與**尚未解碼的** `blocks` 字典(`PackedByteArray` 本身是原生型別,取得它不等於解讀它的內容)——「僅讀取 manifest」路徑到此為止,不對任何區塊的 `PackedByteArray` 再呼叫一次 `bytes_to_var()`;「完整讀取」路徑則對需要的區塊緩衝區各自再呼叫一次 `bytes_to_var(buffer, false)` 才真正取得該區塊的 payload `Dictionary`。
...
1. `bytes_to_var(buffer, false)` 解碼外層結構,取得 `ruleset_version`、`block_manifest`(含逐區塊雜湊)、`top_level_hash`,以及尚未解碼的 `blocks` 字典(僅取得 `PackedByteArray` 參照,不解讀內容)。
...
5. 通過雜湊驗證的區塊,以 `bytes_to_var(buffer, false)` 解碼其 `PackedByteArray`,取得該區塊的 payload `Dictionary`——此呼叫本身即是型別白名單閘門(機制一),解碼失敗(例如位元組本應解碼出一個 Object)視為 `DATA_CORRUPTED`。
...
```
                    ...
                    bytes_to_var(外層 buffer, false) ──▶ ①頂層雜湊比對 ──▶ ②版本比對(VERSION_TOO_NEW 短路)
                                                              │
                                          ┌───────────────────┘
                                          ▼
...
# serialize_block/deserialize_block 內部分別包一層 var_to_bytes()/bytes_to_var(buffer, false)——
...
- **`bytes_to_var(bytes, false)` 的安全失敗行為(Engine Compatibility Verification Required 第 2 項)目前僅為訓練資料推測,尚未經本專案實機驗證**——若實際行為不是「安全失敗」而是其他(例如拋出未分類例外、或部分解碼後回傳不完整結構),機制一的型別安全論證需要重新評估,可能需要在 `bytes_to_var()` 呼叫外再包一層防衛(例如額外的結構完整性檢查——GDScript 無例外機制,故防衛須以回傳值檢查實現)。
...
| 風險 | 緩解 |
|---|---|
| **`bytes_to_var(buffer, false)` 對惡意/損毀位元組的實際行為與訓練資料推測不符**(見 Verification Required 第 2 項) | 本 ADR 明文列為架構階段第一優先驗證項;若實機行為不同,機制三步驟 5 的失敗處置需要調整,但機制二/四(緩衝區分層、雙層雜湊)本身不受影響——雜湊驗證發生在解碼之前(機制三步驟 4 先於步驟 5),即使 `bytes_to_var()` 本身的失敗行為有出入,絕大多數損毀情境已在雜湊比對階段被攔截,不會抵達解碼呼叫 |
...
|---|---|---|
| TR-save-001 | 序列化格式未定案 | 選定 `var_to_bytes()`/`bytes_to_var(buffer, false)` 二進位 Variant 序列化(機制一),拒絕 Resource/JSON(Alternatives 1/2) |
...
4. **`bytes_to_var(buffer, false)` 安全失敗的實機驗證**(對應 Verification Required 第 2 項):構造一段刻意編碼出 Object 型別的位元組,確認解碼回傳可預期的失敗結果(而非崩潰或未定義行為)。
