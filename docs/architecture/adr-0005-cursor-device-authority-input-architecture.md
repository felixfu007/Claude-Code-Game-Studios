...
> | 條件 | 依據 |
> |---|---|
> | 一、引擎行為都在真引擎跑過 | ✅ 六項跑了五項(`prototypes/adr0005-engine-probes-2026-09-01/`)。**#5(Steam 疊加層焦點通知)測不了** —— 本機無 GodotSteam / App ID / 已登入客戶端,已誠實登記並列出要測需要什麼;依門檻文件第四節第 1 項不阻擋(答案最壞的情況是**多一條備援路徑**,不改變設計) |
> | 二、沒有一句話被測試否證 | ✅ 探針未推翻任何一句。**#13 落在危險方向但方向與本 ADR 原本的疑慮一致**,是坐實而非推翻 |
> | 三、全稱句可當場數 | ✅ 2026-09-01 修畢(含一處更正者自己數錯、由窄範圍複驗抓到的) |
> | 四、兩道覆核都跑過 | ✅ 引擎專家已跑;**架構總監(`TD-ADR`)於 2026-09-01 首次執行**,判 CONCERNS 且**零項會改變決定**,修訂經**窄範圍複驗通過** |
> | 五、上游依賴已核准 | ✅ `Depends On: None`。複驗另確認「借用 ADR-0001 兩條禁令先例」不構成依賴 —— 那兩條是通用程式衛生規則,本 ADR 各自重寫了理由,**ADR-0001 即使整份被否決,本 ADR 一行都不用改** |
>
> 🔴 **本 ADR 對實作者的兩條硬性義務**(核准一併生效):
> ①**游標圖層必須獨佔一顆 `CanvasLayer`**,不得與介面圖層共用(誤差實測見 Validation Criteria 第 20 條);
> ②**機制四之二必須自行過濾 `InputEventKey.echo`** —— 引擎的 `event_is_action()` **不過濾**(VR #13 已實測)。
>
> **本次核准明確未涵蓋**(依門檻文件第四節,皆不阻擋核准):滑鼠奪權子機制的兩項已知缺陷
> (**管理者明文凍結,凍結是決定不是缺陷**);Validation Criteria #19 自列的「另開一次全新
...
>
> 🔴 **2026-09-15 管理者追認 —— 上面那條約束的來歷,以及它現在的位階。**
>
> **它原本不是裁決來的。** 上面那句「在第一個 `CursorSurface` 落地並回答該節的問題之前,AC-30 不得被計為『完全涵蓋』」,是 `technical-director` 在 2026-09-15 那次事實層更正(提交 `599108e`)中**自行寫入**的。當日管理者的裁決**逐字只有**「更正架構文件,拿掉那兩個方法」,**沒有要求新增任何約束**。亦即它一度是一條**沒有授權來源的規定** —— 而它寫在一份 `Accepted` 的 ADR 裡,措辭與本文件其他經裁決的內容**看不出任何差別**。
>
> **它是怎麼被發現的**:寫入它的專家在交件時**自己主動標出「這條是我加的,需要管理者確認」**(記於提交 `599108e` 訊息的「專家自陳兩項需要管理者知道的事」第 1 項,以及 `production/session-state/active.md` 當日待辦第 ❹ 項)。**那個自陳是它被發現的唯一途徑** —— 本文件從未帶過「待確認 / 未經裁決」之類的標記,**只讀本文件的人無從察覺它與周圍條文的來歷不同。**
>
> **追認**:**2026-09-15,管理者裁決「留下,並記明今天由管理者追認」**,並明確否決了另外兩個選項(「拿掉 —— 我沒要求過」、「留下但標成未經裁決」)。**依據**:該約束內容方向正確、與 Story 008 既有登記的缺口一致;**拿掉它等於重新打開一個真實的缺口。**
>
> **現在的位階**:**已追認,與本 ADR 其他經裁決的內容同級。** ⚠️ 追認的對象是**上面那句現有的措辭本身**,不是任何修訂版 —— 本次**未改動該約束一字**。
>
> 📌 **本約束在庫內共有四份複本,本節是唯一寫著完整來歷的一份。** 另三份:①機制九下方「🔴 2026-09-15 事實層更正」節(該節自稱缺口登記處,已加短版追認並指回本節);②`docs/registry/architecture.yaml` 的 `cursor_arbitration_suspension_gate` 條目(英文,已加一句追認並指回本節);③`docs/architecture/adr-revision-history.md` 的 2026-09-15 那一列。**③ 刻意不追改** —— 該檔是當日的歷史流水帳,回頭改它等於竄改紀錄;但它今天讀起來同樣像一條已裁決的規定,**讀到該處請以本節為準。**
...
| ⑤ 全域視覺層 | `CursorStateHost` 持有的 CanvasLayer 子節點 | **50** | 奪權漸進回饋載體(自繪游標)、機制十三之二的 hover 判定 —— **2026-08-21 修訂(R6-8):原為三個獨立節點,另含待機指示器**;**2026-09-03 因 TR-cursor-016 取消,降為兩個獨立節點**(見下方 ⑤ 的展開說明);同為呈現層這**一個角色的兩個實例**,彼此無定序需求,故同值 50 不違反「兩兩相異」(該規則管轄角色之間,見 R6-9 的範圍限定)。⚠️ **以下為已被 R6-8 推翻的舊寫法,保留供追溯,不是現行規定**:「同一節點單一 `_process()` 內完成,避免與其他 priority=50 節點的同層排序未定義問題」——拆節點後該理由已失效,現行立場見下方 ⑤ 的展開說明。 |
...
> 🔴 **2026-09-15 管理者追認 —— 上面那條約束原為專家自行寫入,今日已由管理者追認。**
> 它**不是** 2026-09-15 那次裁決要求的(該裁決逐字只有「更正架構文件,拿掉那兩個方法」),而是 `technical-director` 在同一次修訂中**自行寫入**;寫入者在交件報告中自陳需要確認,**本文件從未帶過任何標記**。**2026-09-15 管理者裁決:留下,並追認現有措辭** —— 依據是內容方向正確、與 Story 008 既有登記的缺口一致,拿掉等於重新打開一個真實缺口。**位階:與本 ADR 其他經裁決的內容同級。追認的是上面那句現有措辭本身,一字未改。**
> 📌 **完整來歷、它是怎麼被發現的、以及追認依據,寫在 `## Status` 節的「🔴 2026-09-15 管理者追認」段。本處刻意不複述** —— 同一份文件養出兩份敘述,兩份就會各自漂移。
...
**重入閘門**:`arbitrate_device_authority()`、`apply_buffered_navigation()`、`set_target()`、`mark_pending_reresolve()`、`handoff_before_unload()`、`handoff_after_mount()`、`reseed_reclaim_on_focus_regained()` **七個公開入口**(2026-08-19 第三次修訂由五增為七:R5-1 新增乙分支專用入口、R5-3 新增復焦重新播種入口)皆受 `_mutation_in_progress` 保護——進入時檢查該旗標,若已為真(代表本次呼叫是從本系統剛發出的訊號處理函式內回頭呼叫寫入介面,即重入),**一律回傳 `REJECTED_REENTRANT`,不執行任何狀態變更、不再次發出訊號**,比照 ADR-0001 `authoritative_write_in_progress`(ADR-0001 於 2026-09-09 由 `settlement_in_progress` 改名) 的 reject-on-input 模式。七者進入時皆先設 `_mutation_in_progress = true`,完成全部欄位寫定、且訊號已發出後才設回 `false`。

**三個回傳 `void` 的入口如何表達拒絕(2026-08-19 第四輪修訂補上,上一版把 `arbitrate_frame() -> void` 與三個有回傳值的方法混列而未區分;第三次修訂由兩個增為三個)**:`arbitrate_device_authority()`、`apply_buffered_navigation()` 與 `reseed_reclaim_on_focus_regained()` 回傳 `void`,**不可能**回傳 `REJECTED_REENTRANT`。三者的閘門語意**不完全相同**(🔴 2026-09-03 更正:原文寫「三者的閘門語意是『偵測到重入即整段 no-op、不寫任何欄位、不發任何訊號』」,**這句話對第三個入口已為假**,而它與本段下方的裁決只相隔兩句 —— 改了結論句、沒改支撐它的前提句,正是本專案反覆發生的漂移形狀):`arbitrate_device_authority()` 與 `apply_buffered_navigation()` **確實是整段 no-op**(不寫任何欄位、不發任何訊號);`reseed_reclaim_on_focus_regained()` **不是** —— 依 R6-10 它會寫 `_pending_reseed`、由持閘門者補做,亦即**請求被延後而非丟棄**。這**不違反**本 ADR 的「絕不靜默」紀律,理由是它們的呼叫方只有本系統自己的兩個節點(`CursorStateHost` 與 `CursorNavigationApplier`),不是外部呼叫方——沒有一個下游系統會因為讀不到拒絕碼而做出錯誤補救。**但重入若真的發生,代表本系統內部有一條非預期的呼叫路徑**,因此三者在**偵測到重入時**應遞增一個 `diagnostic_reentrant_rejection_count`(QA-only,比照機制十五慣例),讓它在測試中可見而非完全無痕。🔴 **2026-09-03 管理者裁決 —— 原文寫「在 no-op 時」,而 R6-10 之後 `reseed_reclaim_on_focus_regained()` 重入時已不再 no-op**(改記 `_pending_reseed`,由持閘門者補做),條件字面上不再成立,而本 ADR 從未回頭說明。**裁決:仍然遞增。** 計數器的語意界定為「偵測到一次不該發生的重入呼叫」,**不是**「請求被丟棄」—— 依據是本段自己寫下的目的(「讓它在測試中可見而非完全無痕」),而請求被延後補做仍然是一次不該發生的呼叫。⚠️ 名稱裡的 `rejection` 因此略有誤導(該次請求並未被拒絕),**但不改名**:改名會波及已寫好的程式與測試,而語意已在此明文界定。📌 **連帶後果(裁決的實際理由)**:此計數器是目前**唯一**能證明 `reseed_reclaim_on_focus_regained()` 的閘門存在的觀測點,不遞增等於該入口的閘門沒有任何測試證據。實作已如此(`src/ui/cursor/cursor_state.gd`),本次為文件補正,非行為變更。四個有回傳值的公開入口(`set_target`/`mark_pending_reresolve`/`handoff_before_unload`/`handoff_after_mount`)則一律回傳 `REJECTED_REENTRANT`,不變。
...
| 路徑 | 成員 | 是否掛閘門 |
|---|---|---|
| **公開入口(七個,2026-08-19 第三次修訂由五增為七)** | `arbitrate_device_authority()`、`apply_buffered_navigation()`、`set_target()`、`mark_pending_reresolve()`、`handoff_before_unload()`、**`handoff_after_mount()`**、**`reseed_reclaim_on_focus_regained()`** | **是**。進入設旗標、離開清旗標,重入回傳 `REJECTED_REENTRANT`(三個 `void` 入口則為診斷計數 + no-op,**但 `reseed_reclaim_on_focus_regained()` 例外:依 R6-10 是延後補做而非 no-op**,見機制十該段 2026-09-03 更正) |
...
1. **GDD 明文,不是推論** —— Core Rules #7 情境(乙)原文逐字寫著乙分支
   「**與「新開局進入戰鬥」走完全相同**」。**設計文件自己就要求兩者同路。**
   加上 AC-62 的可測試條款「對本系統介面的**呼叫序列在兩條路徑上完全相同**」,
   與本節 R5-1 已窮舉證明的「乙分支別無選擇,只能走 `handoff_after_mount()`」,
   三者相加是一個**構造性證明**:新開局若走 `set_target()`,兩條路徑呼叫的是不同方法名,
   AC-62 在字面上就不可能成立 —— 一條已 Approved 的驗收標準會變成永遠紅燈的廢文。

   ⚠️ **本項原本寫的不是這個。** 2026-09-04 初稿引的是 AC-62 的設計意圖散文
   「複用 Core Rules #6 既有流程」,並據以推論「複用的對象就是新開局那條流程」。
   **同日 `godot-specialist` 對抗性覆核判定該推論為過度推論** —— 「複用」在中文可讀成
   「沿用同一條**規則**」而非「沿用同一個**入口**」,支撐不起這個結論。
   **上面那句 Core Rules #7 的原文才是決定性依據,而初稿沒有引到它。**
   保留這段記錄,是因為「結論對、理由弱」與「結論對、理由硬」對下一個重新檢視的人不是同一件事。
...
3. 🔴 **代價不是零 —— 是「另一個選項才有非零風險」**(2026-09-04 覆核更正,方向不變、論證更換)。

   初稿寫「對一局全新遊戲而言兩個入口同義,選擇代價為零」。**覆核找到一個具體反例**:
   若「新開局」在**同一次程式執行中**可能發生第二次(玩家中途返回標題再開新局,未重啟程式),
   而**棋盤地形是手工設計、程序化生成是專案級禁令** —— 每次新開局算出的初始格**很可能是同一個值**。
   此時走 `set_target()`,其 `CONDITIONAL_ON_CHANGE` 語意會在「新目標恰等於當下目標」時
   **靜默跳過重置** —— **正是本節 R5-1 為乙分支修掉的那個 bug,換一扇門進來。**

   ⚠️ **「新開局在單次程式執行中會不會發生第二次」查不到權威依據**(游標系統文件不管遊戲流程)。
   **但這個不確定性本身就是選 `handoff_after_mount()` 的理由**:它的無條件語意對這個問題免疫,
   不必先回答它。

   ✅ **同批查證並排除一項疑慮**:曾擔心 `handoff_after_mount()` 拒絕未註冊表面而
   `set_target()` 不會,因而新增一項「必須先註冊表面」的呼叫方義務。**前提為假** ——
   兩者走的是同一個私有驗證路徑(`cursor_state.gd` 的 `_validate_target_writable()`),
   `write_read_interface_test.gd` 已有測試斷言兩者對同一個未註冊表面回傳同一個拒絕碼。
   **兩個入口本來就同樣要求表面已註冊,選哪一個都不增加義務。** 記在此處以免再被提出一次。
...
✅ **`cursor_state.gd` 的 `handoff_after_mount()` 方法註解已於 2026-09-04 回補本裁決**
(覆核發現:該註解原本只寫「讀檔成功後呼叫」,而寫新流程的人會先讀它、不會先翻本檔,
照它讀下來會得出「新開局不涉及讀檔,不該用這個」—— **正是本裁決要防的錯誤讀法**)。

---

#### 🔴 本節的覆核履歷(2026-09-04,補跑 `TD-ADR` 缺口)

**本節初稿由協調者撰寫、未經任何架構覆核即寫入一份 `Accepted` ADR。** 管理者當日察覺此缺口,
指派 `godot-specialist` 做**對抗性**審查(明文要求試著推翻,不是確認)。

**結論:裁決成立。** 但三項理由**有兩項被更換或更正**(見上方各該項的 ⚠️ 段),
且覆核另外找出兩項初稿沒看到的東西:①`cursor_state.gd` 註解未回補本裁決(已修);
②GDD 該句「不需要新介面」已被 R5-1 推翻,登記為 ADVISORY(`docs/tech-debt-register.md`)。

📌 **記在這裡的理由**:本專案有明文紀錄指出 `TD-ADR` 覆核關卡「規則存在、無人執行、
且不執行不留痕」,**九次 ADR 修訂全部跳過**。本節是第十次跳過 —— **差別只在它被補跑了,
而且留下了這行紀錄。** 下一個修訂本 ADR 的人:**這道關卡是存在的。**
...
#     **七個公開入口**在清除 _mutation_in_progress **之前**各呼叫本方法恰一次
#     (2026-09-03 事實層更正:原寫「六個會發訊號的公開入口」,實測七個全部都呼叫。
#      同一句話在驗證要求 #13 (iv) 有另一份複本,已同批更正。)
...
16. **R5-3 擁有權與轉發鏈測試(2026-08-19 第三次修訂新增)**:(i) 靜態斷言 `CursorState` **不存在**任何回傳 `MouseReclaimPolicy` 的公開方法或屬性(`_reclaim` 不外流);(ii) 斷言 `CursorStateHost` 全文不出現 `_reclaim`,且 `get_viewport().get_mouse_position()` 在全專案**恰好出現一次**(建構 `mouse_position_provider` 之處);(iii) 對自繪節點斷言其只依賴 `CursorState.reclaim_reset_triggered` 與 `CursorState.reclaim_progress()`,不持有 `MouseReclaimPolicy` 參照;(iv) **重入向量(專家發現 D)**:於 `target_changed()` 的處理函式內同步呼叫 `CursorStateHost.resume_arbitration()`,🔴 **2026-09-03 改寫。原文斷言「`_reclaim` 在該次外層寫入內**只被 reset 一次**」,而 R6-10 之後實際是兩次(外層寫入自己的 reset + 補做的 `FOCUS_LOST_REGAINED`)。原文是修訂前的舊複本 —— 照它字面寫測試的人會得到一條錯的紅燈,並可能回頭去「修好」一個正確的實作。** 本項在四處文件自我矛盾中被判為**最急**,正是因為其餘三處只是讓人漏檢,只有這一處會讓人主動破壞正確的東西。**改為斷言順序與不變式,不斷言次數** —— 次數隨外層入口而異(🔴 2026-09-03 覆核更正:此處原寫「實測 2/2/2/1/0」,覆核者逐一推導為 **{2,2,2,2,1,0,0}** —— 漏掉的那個 2 是 `reseed_reclaim_on_focus_regained()` 自己,它不經由 `target_changed()` 而是經由轉發的 `reclaim_reset_triggered` 誘發重入,**只盯著 `target_changed()` 找路徑就會剛好漏掉它**;另兩個 0 只因 Story 005 兩個接縫未實作,註定會變。**原數字未附任何執行檔路徑,依本專案 (A) 級定義本就不成立,故此處不再寫任何數字**),斷言次數必然再度過期。四條:(a) 補做**確實發生**,且該次 `reset()` 帶 `FOCUS_LOST_REGAINED`;(b) 補做發生在**清除 `_mutation_in_progress` 之前**(外層入口返回前),不是被丟棄、也不是延到下一影格。🔴 **2026-09-03 覆核指出:「返回前」與「清旗標前」之間有一小段,而 `_mutation_in_progress` 是私有的,單看 `triggers` 無法區分** —— 把 drain 與清旗標**對調**的錯誤實作會產生一模一樣的觀測結果。**唯一能區分的觀測點**:在 `reclaim_reset_triggered` 的處理函式內呼叫一個**有回傳值**的閘門入口,斷言得到 `REJECTED_REENTRANT`(旗標若已清除就會得到 `APPLIED`);(c) `diagnostic_reentrant_rejection_count` **遞增**(2026-09-03 管理者裁決,全文見機制十該段);(d) 外層寫入自己的 reset 與補做的 reset **觸發點不同**,不得合併或互相取代。🔴 **2026-09-03 覆核指出:用「包含 `FOCUS_LOST_REGAINED`」斷言不夠** —— 一個**丟掉外層 reset** 的實作照樣通過。應斷言**恰好等於** `[TARGET_CHANGED, FOCUS_LOST_REGAINED]`(有序且無多餘),一次同時涵蓋 (d)、兩者的先後、以及次數的上界。
...
20. 🔴 **游標圖層變換恆等(2026-09-01 畫面架構裁決導出,`TD-ADR` 覆核補入)**:斷言
    ...
    **那正是這一條當初被寫進來要對治的那件事。**
    ...
    ✅ **本條已於 2026-09-04 由 Story 010 落地。但落地方式與上一段的指示不同,
    而且理由是上一段的前提被實測推翻 —— 不是便宜行事,請讀完再判斷。**

    **上一段的前提**是「換解析度、量圖層變換**是開真實視窗跑出來的**」,因此只能放在
    人工測試線上,因此必須明文指定為有視窗的整合測試,否則會退化成沒人跑。

    🔴 **那個前提實測為假。** 開工前探針(`prototypes/story-010-headless-resolution-probe-2026-09-04/`,
    逐字 log 在該目錄 `logs/probe_output.txt`)量到:**headless 確實套用 content scale**,
    四種解析度下根視窗 `get_final_transform()` 分別為 4× / 5× / 8× / 5×,
    2K 偏移 `(80, 45)`、超寬偏移 `(520, 45)` —— **與 2026-09-01 那支非 headless、
    真實 GPU 的 spike 逐項一致**。亦即這條斷言**量得到,而且量得準**,不需要真實視窗。

    **因此本條反而以更強的形式被滿足**:它現在跑在**每次推送都會自動執行**的測試線上
    (`tests/unit/cursor/cursor_layer_transform_test.gd`),而不是一次人工執行。
    **上一段真正擔心的事(「變成只有人手動跑一次」)被消除了,而不是被繞過。**

    ✅ **管理者裁決(2026-09-04)**:接受 headless 落地,**但條件是「真的開視窗用眼睛看畫面」
    的義務移交 Story 011** —— 那是第一張會真的畫出東西的工作單,而 headless 抓不到的正是
    「要畫出來才看得見」那一類。該義務已寫入 `story-011-native-cursor-suppression.md` 的
    執行位置(不是寫在已結案的 Story 010 裡)。

    📌 **實作差異兩點,誠實記錄**:
    ① 本條原文寫「1080p / 2K / 4K **三種**」,實際交付**四種**(加超寬 3440×1440)——
    超集,非缺漏。② 交付含一條靈敏度測試與四次逐條注入量測,證明這四條斷言在圖層真的被
    弄壞時**會紅**(觀測,非推論);方法與逐條數字見
    `production/qa/evidence/cursor-canvas-layer-evidence.md`。

    🔴 **寫本測試的人必讀的一個陷阱**:`DisplayServer.window_set_size()` 在 headless
    **靜默無作用且不報錯**(四種解析度下 `window_get_size()` 一律回傳 `(0, 0)`)。
    照直覺用它寫的測試會**四個迴圈跑在同一尺寸上然後四條全部通過**。
    **解析度一律指派 `get_tree().root.size`。**

    ⚠️ **本註記的由來也記在這裡**:2026-09-04 協調者把「要不要接受 headless」呈給管理者時,
    依據的是實作者報告裡的一行轉述,**沒有打開本段原文**,因此選項描述寫成「我們偏離了 ADR」,
    而非「我們量到 ADR 的前提是錯的」。裁決結果相同,但呈報時低估了已知資訊。
    見 `docs/consistency-failures.md` 2026-09-04 條目。
