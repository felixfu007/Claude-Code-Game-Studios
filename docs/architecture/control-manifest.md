...
> **最後更新**:2026-09-09
> **清單版本**:2026-09-09
> **涵蓋 ADR**:ADR-0001(**第一次修訂 2026-09-09**)、ADR-0002、ADR-0003、ADR-0005(**四份 `Accepted`**)
> **狀態**:Active —— ADR 有異動時以 `/create-control-manifest update` 重新生成

> 🔴 **2026-09-09 變更摘要 —— 若你手上的 story 嵌的是 2026-09-02 版,以下三條你拿到的是錯的:**
> ① `board_version` 已改名並移交 → **`BattleState.combat_state_version`**,且它背書的不只棋盤,
> 含單位攻防與行動旗標;② `settlement_in_progress` 已改名 → **`authoritative_write_in_progress`**,
> 並升格為**寫入守衛**;③ `is_stale(board)` → **`is_stale()`(不帶參數)**。
> **舊版第一條規則會讓打牌後畫面顯示舊傷害,而照著寫的人不會做錯任何事。**
...
> 🔴 **2026-09-09 本節重寫(ADR-0001 第一次修訂)。舊版的第一條規則是錯的,而照它寫的人
> 不會做錯任何事就會做出一個缺陷。** 舊版寫「`board_version` 只在已提交的結算邊界完成時 +1」——
> **打牌不是結算,照這條寫就不會遞增,於是打牌前算的「傷害 4」會通過過期檢查、繼續顯示,
> 而實際打出 7,且不會有任何錯誤訊息。** 三個識別字全部改名/移交,舊名不再有效。

- 🔴 **`combat_state_version` 由 `BattleState` 持有,只由 `commit_authoritative_change()` 遞增。**
  **不是** `Board.board_version`(2026-09-09 前的舊名與舊位置,已作廢)
- 🔴 **它背書三個零件**:`Board`(地形/佔位)、`Unit`(攻防/HP)、`TurnOrder`(行動旗標/陣營/回合數)。
  **不只棋盤格資料** —— 卡牌改的攻防值也在背書範圍內
- 🔴 **凡改變上述任一零件的已提交寫入,完成時一律 +1,無條件。** 已登記的路徑至少六條:
  ①玩家確認指令的結算步 ②已確認的移動邏輯 ③敵方回合整批結算 ④玩家主動結束單位行動
  ⑤玩家結束陣營回合 ⑥**卡牌打出第 4 步確認**。
  **不得**依卡牌類別或路徑種類判斷是否跳過
- 🔴 **`BattleController` 與 `BattleLoop` 是兩條平行的驅動路徑,互不呼叫。**
  提交方法必須是兩者**共用的同一個**,不得各自複製一份
- **唯讀操作一律不得使其遞增**:游標移動、開關疊加圖、預判標記、**單位選取/取消選取**、
  打牌流程第 1~3 步(開手牌/選牌/選對象)後取消
- **每個查詢結果攜帶它所計算的版本號**;有效性判準以 **`result.is_stale()`(不帶參數)** 為準。
  ⚠️ **不是** `is_stale(board)`(舊簽章,2026-09-09 作廢)—— 舊簽章允許傳錯一個 `Board`
  而不報錯
...
- 🔴 **`authoritative_write_in_progress`(舊名 `settlement_in_progress`,2026-09-09 作廢)
  於任一寫入路徑開始時設 `true`、完成後設 `false`,並於此時遞增 `combat_state_version`。**
  結算步的具體對應為①開始、④完成
- 🔴 **寫入守衛:三個零件的每一個 mutator,第一行檢查該旗標;窗口未開就 `push_error()` 並
  拒絕寫入,不得靜默略過。** ⚠️ **把寫入口收攏成單一方法「不是」結構保證** ——
  GDScript 沒有存取修飾詞,收攏擋不住任何人繞過去。**會擋下來的是這條守衛。**
- 🔴 **`Unit` 的 `hp`/`atk`/`def`/`mp`/`min_range`/`max_range` 現在是裸公開 `var`,
  必須先長出 setter,守衛才有地方放。** 這一層不存在,上一條就是空話
- 🔴 **`Board.set_occupant()` / `clear_occupant()` 收進提交方法內部,不再對外公開**
  (現況是公開方法,且 `battle_state.gd` 有四處直接呼叫)
- 🔴 **提交包裝跑在 `_input` 或 `_process` 皆可(2026-09-09 實測改為條件式等價),
  但三個條件缺一不可**:①該幀至多一個會觸發該路徑的合法輸入事件(`_input` 是逐事件呼叫,
  鍵盤與手把同幀各送一次確認會跑兩次 —— **須自帶去重**);②不依賴 `process_priority`
  或跨節點 `_input` 的相對順序;③🔴 **不查詢任何要到 `_process` 才算出的狀態,
  尤其 `CursorStateHost` 的裝置權威與當前目標** —— 在 `_input` 裡讀它會讀到**上一幀**的值,
  且不報錯。**卡牌介面必然踩到條件③**(其 UI 規格要求選取視覺走單一游標狀態源)
- ⚠️ **`_physics_process` 的禁令不受上條放寬影響,仍然絕對**:放進去會讓 ADR-0005 的全部
  定序保證**靜默失效且不報錯**
- **寫入進行中,所有玩家輸入一律拒絕並觸發拒絕回饋**(不採佇列)
- **`authoritative_write_in_progress` 不得跨越兩個連續的 `_process` 幀仍為 `true`**;偵測到須以
  `push_error()` 明確曝光(可直接寫成自動化測試)。
  📌 **這條同時是守衛的終止條件** —— 一個被非法開啟的寫入窗口,兩幀之內一定會叫
- **`occupied` 由稀疏 `Dictionary[Vector2i, int]` 承載**,只記錄有單位的格
- **佔位同步與 `combat_state_version` 遞增在同一個原子區段內完成**
- **查詢結果攜帶的容器必須是新配置的物件**
